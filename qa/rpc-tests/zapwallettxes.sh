...
if [ $# -lt 1 ]; then
        ...
        echo "e.g. $0 ../../src"
        echo "Env vars OACCOIND and OACCOINCLI may be used to specify the exact binaries used"
        exit 1
fi

set -f

OACCOIND=${OACCOIND:-${1}/oaccoind}
CLI=${OACCOINCLI:-${1}/oaccoin-cli}
...
$OACCOIND -zapwallettxes=1 $B1ARGS &
B1PID=$!
$OACCOIND -zapwallettxes=2 $B2ARGS &
B2PID=$!
