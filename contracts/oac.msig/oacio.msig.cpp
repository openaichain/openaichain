#include <oac.msig/oac.msig.hpp>
#include <oaclib/action.hpp>
#include <oaclib/permission.hpp>
...
void multisig::propose() {
   ...
   oac_assert( trx_header.expiration >= oac::time_point_sec(now()), "transaction expired" );
   //oac_assert( trx_header.actions.size() > 0, "transaction must have at least one action" );

   proposals proptable( _self, proposer );
   oac_assert( proptable.find( proposal_name ) == proptable.end(), "proposal with the same name exists" );
   ...
   oac_assert( res > 0, "transaction authorization failed" );
...
void multisig::approve( account_name proposer, name proposal_name, permission_level level ) {
   ...
   oac_assert( itr != apps.requested_approvals.end(), "approval is not on the list of requested approvals" );
...
void multisig::unapprove( account_name proposer, name proposal_name, permission_level level ) {
   ...
   oac_assert( itr != apps.provided_approvals.end(), "no approval previously granted" );
...
void multisig::cancel( account_name proposer, name proposal_name, account_name canceler ) {
   ...
   if( canceler != proposer ) {
      oac_assert( unpack<transaction_header>( prop.packed_transaction ).expiration < oac::time_point_sec(now()), "cannot cancel until expiration" );
   }
...
OACIO_ABI( oac::multisig, (propose)(approve)(unapprove)(cancel)(exec) )
