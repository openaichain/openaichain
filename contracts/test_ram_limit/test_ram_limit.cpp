...
class test_ram_limit : public oac::contract {
   public:
      ...
      void setentry(account_name payer, uint64_t from, uint64_t to, uint64_t size) {
         ...
         oac::print("test_ram_limit::setentry ", oac::name{self}, "\n");
      ...
      void rmentry(uint64_t from, uint64_t to) {
         const auto self = get_self();
         oac::print("test_ram_limit::rmentry ", oac::name{self}, "\n");
         test_table table(self, self);
         for (int key = from; key <=to; ++key) {
            auto itr = table.find(key);
            oac_assert (itr != table.end(), "could not find test_table entry");
            table.erase(itr);
         }
      }
      ...
      void printentry(uint64_t from, uint64_t to) {
         const auto self = get_self();
         oac::print("test_ram_limit::printout ", oac::name{self}, ":");
         test_table table(self, self);
         for (int key = from; key <=to; ++key) {
            auto itr = table.find(key);
            oac::print("\nkey=", key);
            oac_assert (itr != table.end(), "could not find test_table entry");
            oac::print(" size=", itr->data.size());
         }
      }
   ...
   private:
      struct test {
         ...
         OACLIB_SERIALIZE( test, (key)(data) )
      };
      typedef oac::multi_index< N(test.table), test> test_table;
};

#pragma clang diagnostic pop

OACIO_ABI( test_ram_limit, (setentry)(rmentry)(printentry) )
