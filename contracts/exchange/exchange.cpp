...
namespace oac {
   ...
   void exchange::withdraw( account_name from, extended_asset quantity ) {
      require_auth( from );
      oac_assert( quantity.is_valid(), "invalid quantity" );
      oac_assert( quantity.amount >= 0, "cannot withdraw negative balance" ); // Redundant? inline_transfer will fail if quantity is not positive.
   ...
   void exchange::on( const trade& t ) {
      ...
      oac_assert( t.sell.is_valid(), "invalid sell amount" );
      oac_assert( t.sell.amount > 0, "sell amount must be positive" );
      oac_assert( t.min_receive.is_valid(), "invalid min receive amount" );
      oac_assert( t.min_receive.amount >= 0, "min receive amount cannot be negative" );

      auto receive_symbol = t.min_receive.get_extended_symbol();
      oac_assert( t.sell.get_extended_symbol() != receive_symbol, "invalid conversion" );
      ...
      if( t.min_receive.amount != 0 ) {
         oac_assert( t.min_receive.amount <= output.amount, "unable to fill" );
      }
   ...
   void exchange::on( const upmargin& b ) {
      ...
      oac_assert( b.delta_borrow.is_valid(), "invalid borrow delta" );
      oac_assert( b.delta_collateral.is_valid(), "invalid collateral delta" );

      market_state market( _this_contract, b.market, _accounts );

      oac_assert( b.delta_borrow.amount != 0 || b.delta_collateral.amount != 0, "no effect" );
      oac_assert( b.delta_borrow.get_extended_symbol() != b.delta_collateral.get_extended_symbol(), "invalid args" );
      oac_assert( market.exstate.base.balance.get_extended_symbol() == b.delta_borrow.get_extended_symbol() ||
                    market.exstate.quote.balance.get_extended_symbol() == b.delta_borrow.get_extended_symbol(),
                    "invalid asset for market" );
      oac_assert( market.exstate.base.balance.get_extended_symbol() == b.delta_collateral.get_extended_symbol() ||
   ...
   void exchange::on( const covermargin& c ) {
      ...
      oac_assert( c.cover_amount.is_valid(), "invalid cover amount" );
      oac_assert( c.cover_amount.amount > 0, "cover amount must be positive" );
   ...
   void exchange::createx( account_name    creator,
      ...
      oac_assert( initial_supply.is_valid(), "invalid initial supply" );
      oac_assert( initial_supply.amount > 0, "initial supply must be positive" );
      oac_assert( base_deposit.is_valid(), "invalid base deposit" );
      oac_assert( base_deposit.amount > 0, "base deposit must be positive" );
      oac_assert( quote_deposit.is_valid(), "invalid quote deposit" );
      oac_assert( quote_deposit.amount > 0, "quote deposit must be positive" );
      oac_assert( base_deposit.get_extended_symbol() != quote_deposit.get_extended_symbol(),
      ...
      oac_assert( existing == exstates.end(), "market already exists" );
   ...
   void exchange::lend( account_name lender, symbol_type market, extended_asset quantity ) {
      require_auth( lender );
      oac_assert( quantity.is_valid(), "invalid quantity" );
      oac_assert( quantity.amount > 0, "must lend a positive amount" );
   ...
   void exchange::unlend( account_name lender, symbol_type market, double interest_shares, extended_symbol interest_symbol ) {
      require_auth( lender );
      oac_assert( interest_shares > 0, "must unlend a positive amount" );
   ...
   void exchange::on( const currency::transfer& t, account_name code ) {
      ...
      if( t.to == _this_contract ) {
         ...
         oac_assert( a.is_valid(), "invalid quantity in transfer" );
         oac_assert( a.amount != 0, "zero quantity is disallowed in transfer");
         oac_assert( a.amount > 0 || t.memo == "withdraw", "withdrew tokens without withdraw in memo");
         oac_assert( a.amount < 0 || t.memo == "deposit", "received tokens without deposit in memo" );
   ...
   void exchange::apply( account_name contract, account_name act ) {
      ...
      switch( act ) {
         OACIO_API( exchange, (createx)(deposit)(withdraw)(lend)(unlend) )
      };
