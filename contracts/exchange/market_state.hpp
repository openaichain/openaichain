...
namespace oac {
   ...
   struct margin_position {
      ...
      OACLIB_SERIALIZE( margin_position, (owner)(borrowed)(collateral)(call_price) )
   };

   typedef oac::multi_index<N(margins), margin_position,
           indexed_by<N(callprice), oac::const_mem_fun<margin_position, uint64_t, &margin_position::get_call> >
   > margins;
   ...
   struct loan_position {
      ...
      OACLIB_SERIALIZE( loan_position, (owner)(interest_shares) )
   };

   typedef oac::multi_index<N(loans), loan_position> loans;
