...
#include <oaclib/vector.hpp>

namespace identity {
   using oac::action_meta;
   using oac::singleton;
   using oac::key256;
   using std::string;
   using std::vector;
      ...
      public:
        ...
        using identity_base::identity_base;
         ...
         void create( const account_name creator, const uint64_t identity ) {
            ...
            oac_assert( itr == t.end(), "identity already exists" );
            oac_assert( identity != 0, "identity=0 is not allowed" );
                               ...
                               const vector<certvalue>& values )
         {
            ...
            idents_table t( _self, _self );
            oac_assert( t.find( identity ) != t.end(), "identity does not exist" );
            ...
            for( const auto& value : values ) {
               ...
               if (value.confidence) {
                  oac_assert(value.type.size() <= 32, "certrow::type should be not longer than 32 bytes");
                  ...
                  if (value.property == N(owner)) {
                     oac_assert(sizeof(account_name) == value.data.size(), "data size doesn't match account_name size");
...
OACIO_ABI( identity::contract, (create)(certprop)(settrust) );
