...
namespace oac {
   ...
   void set_blockchain_parameters(const oac::blockchain_parameters& params) {
      char buf[sizeof(oac::blockchain_parameters)];
      oac::datastream<char *> ds( buf, sizeof(buf) );
      ds << params;
      set_blockchain_parameters_packed( buf, ds.tellp() );
   }

   void get_blockchain_parameters(oac::blockchain_parameters& params) {
      char buf[sizeof(oac::blockchain_parameters)];
      size_t size = get_blockchain_parameters_packed( buf, sizeof(buf) );
      oac_assert( size <= sizeof(buf), "buffer is too small" );
      oac::datastream<const char*> ds( buf, size_t(size) );
      ds >> params;
   }
   ...
   */
   ...
   private:
      ...
      public:
         ...
         {
            oac_assert(_offset == _heap_size, "malloc_from_freed was designed to only be called after _heap was completely allocated");
...
{
   return oac::memory_heap.malloc(size);
}

void* calloc(size_t count, size_t size)
{
   void* ptr = oac::memory_heap.malloc(count*size);
...
{
   return oac::memory_heap.realloc(ptr, size);
}

void free(void* ptr)
{
   return oac::memory_heap.free(ptr);
}
