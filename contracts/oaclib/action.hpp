...
namespace oac {
    ...
    *    int  c; //4
    *
    *    OACLIB_SERIALIZE( dummy_action, (a)(b)(c) )
    *  };
   ...
   struct permission_level {
      ...
      OACLIB_SERIALIZE( permission_level, (actor)(permission) )
   };
   ...
   struct action {
      ...
      OACLIB_SERIALIZE( action, (account)(name)(authorization)(data) )
      ...
      void send_context_free() const {
         oac_assert( authorization.size() == 0, "context free actions cannot have authorizations");
         auto serialize = pack(*this);
      ...
      template<typename T>
      T data_as() {
         oac_assert( name == T::get_name(), "Invalid name" );
         oac_assert( account == T::get_account(), "Invalid account" );
...
::oac::inline_dispatcher<decltype(&CONTRACT_CLASS::FUNCTION_NAME), ACTION_NAME>::call

#define INLINE_ACTION_SENDER2( CONTRACT_CLASS, NAME )\
INLINE_ACTION_SENDER3( CONTRACT_CLASS, NAME, ::oac::string_to_name(#NAME) )
...
#define ACTION( CODE, NAME ) struct NAME : ::oac::action_meta<CODE, ::oac::string_to_name(#NAME) >
