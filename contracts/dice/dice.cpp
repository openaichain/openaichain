...
class dice : public oac::contract {
   public:
      ...
      void offerbet(const asset& bet, const account_name player, const checksum256& commitment) {
         ...
         oac_assert( bet.symbol == CORE_SYMBOL, "only core token allowed" );
         oac_assert( bet.is_valid(), "invalid bet" );
         oac_assert( bet.amount > 0, "must bet positive quantity" );

         oac_assert( !has_offer( commitment ), "offer with this commitment already exist" );
         require_auth( player );

         auto cur_player_itr = accounts.find( player );
         oac_assert(cur_player_itr != accounts.end(), "unknown account");
         ...
         if( matched_offer_itr == idx.end()
            ...
            accounts.modify( cur_player_itr, 0, [&](auto& acnt) {
               oac_assert( acnt.oac_balance >= bet, "insufficient balance" );
               acnt.oac_balance -= bet;
               acnt.open_offers++;
            });
            ...
            auto game_itr = games.emplace(_self, [&](auto& new_game){
               ...
               new_game.deadline = oac::time_point_sec(0);
      ...
      void canceloffer( const checksum256& commitment ) {
         ...
         oac_assert( offer_itr != idx.end(), "offer does not exists" );
         oac_assert( offer_itr->gameid == 0, "unable to cancel offer" );
         require_auth( offer_itr->owner );

         auto acnt_itr = accounts.find(offer_itr->owner);
         accounts.modify(acnt_itr, 0, [&](auto& acnt){
            acnt.open_offers--;
            acnt.oac_balance += offer_itr->bet;
         });
      ...
      void reveal( const checksum256& commitment, const checksum256& source ) {
         ...
         oac_assert(curr_revealer_offer != idx.end(), "offer not found");
         oac_assert(curr_revealer_offer->gameid > 0, "unable to reveal");
         ...
         oac_assert( is_zero(curr_reveal.reveal) == true, "player already revealed");
         ...
         if( !is_zero(prev_reveal.reveal) ) {
            ...
            games.modify(game_itr, 0, [&](auto& game){
               ...
               game.deadline = oac::time_point_sec(now() + FIVE_MINUTES);
            });
         }
      }
      ...
      void claimexpired( const uint64_t gameid ) {
         ...
         oac_assert(game_itr != games.end(), "game not found");
         oac_assert(game_itr->deadline != oac::time_point_sec(0) && oac::time_point_sec(now()) > game_itr->deadline, "game not expired");
         ...
         if( !is_zero(game_itr->player1.reveal) ) {
            oac_assert( is_zero(game_itr->player2.reveal), "game error");
            pay_and_clean(*game_itr, *player1_offer, *player2_offer);
         } else {
            oac_assert( is_zero(game_itr->player1.reveal), "game error");
      ...
      void deposit( const account_name from, const asset& quantity ) {
         ...
         oac_assert( quantity.amount > 0, "must deposit positive quantity" );
         ...
         action(
            ...
            N(oac.token), N(transfer),
            std::make_tuple(from, _self, quantity, std::string(""))
         ).send();

         accounts.modify( itr, 0, [&]( auto& acnt ) {
            acnt.oac_balance += quantity;
         });
      }
      ...
      void withdraw( const account_name to, const asset& quantity ) {
         ...
         oac_assert( quantity.is_valid(), "invalid quantity" );
         oac_assert( quantity.amount > 0, "must withdraw positive quantity" );

         auto itr = accounts.find( to );
         oac_assert(itr != accounts.end(), "unknown account");

         accounts.modify( itr, 0, [&]( auto& acnt ) {
            oac_assert( acnt.oac_balance >= quantity, "insufficient balance" );
            acnt.oac_balance -= quantity;
         });
   ...
   private:
      ...
      struct offer {
         ...
         OACLIB_SERIALIZE( offer, (id)(owner)(bet)(commitment)(gameid) )
      };

      typedef oac::multi_index< N(offer), offer,
      ...
      struct player {
         ...
         OACLIB_SERIALIZE( player, (commitment)(reveal) )
      };
      ...
      struct game {
         uint64_t id;
         asset    bet;
         oac::time_point_sec deadline;
         player   player1;
         player   player2;

         uint64_t primary_key()const { return id; }

         OACLIB_SERIALIZE( game, (id)(bet)(deadline)(player1)(player2) )
      };

      typedef oac::multi_index< N(game), game> game_index;
      ...
      struct global_dice {
         ...
         OACLIB_SERIALIZE( global_dice, (id)(nextgameid) )
      };

      typedef oac::multi_index< N(global), global_dice> global_dice_index;
      ...
      struct account {
         ...
         bool is_empty()const { return !( oac_balance.amount | open_offers | open_games ); }

         uint64_t primary_key()const { return owner; }

         OACLIB_SERIALIZE( account, (owner)(oac_balance)(open_offers)(open_games) )
      };

      typedef oac::multi_index< N(account), account> account_index;
...
OACIO_ABI( dice, (offerbet)(canceloffer)(reveal)(claimexpired)(deposit)(withdraw) )
