...
struct impl {
   ...
   void on(const create& c) {
      require_auth(c.host);
      oac_assert(c.challenger != c.host, "challenger shouldn't be the same as host");

      // Check if game already exists
      games existing_host_games(code_account, c.host);
      auto itr = existing_host_games.find( c.challenger );
      oac_assert(itr == existing_host_games.end(), "game already exists");
   ...
   void on(const restart& r) {
      ...
      oac_assert(itr != existing_host_games.end(), "game doesn't exists");

      // Check if this game belongs to the action sender
      oac_assert(r.by == itr->host || r.by == itr->challenger, "this is not your game!");
   ...
   void on(const move& m) {
      ...
      oac_assert(itr->winner == N(none), "the game has ended!");
      // Check if this game belongs to the action sender
      oac_assert(m.by == itr->host || m.by == itr->challenger, "this is not your game!");
      // Check if this is the  action sender's turn
      oac_assert(m.by == itr->turn, "it's not your turn yet!");


      // Check if user makes a valid movement
      oac_assert(is_valid_movement(m.mvt, *itr), "not a valid movement!");
   ...
   void apply( uint64_t /*receiver*/, uint64_t code, uint64_t action ) {
      ...
      if (code == code_account) {
         if (action == N(create)) {
            impl::on(oac::unpack_action_data<tic_tac_toe::create>());
         } else if (action == N(restart)) {
            impl::on(oac::unpack_action_data<tic_tac_toe::restart>());
         } else if (action == N(close)) {
            impl::on(oac::unpack_action_data<tic_tac_toe::close>());
         } else if (action == N(move)) {
            impl::on(oac::unpack_action_data<tic_tac_toe::move>());
         }
      }
   }
