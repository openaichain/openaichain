...
BOOST_FIXTURE_TEST_CASE( basic_test, TESTER ) try {
   ...
   transaction_id_type yes_assert_id;
   {
      ...
      BOOST_CHECK_THROW(push_transaction( trx ), oac_assert_message_exception);
   }
...
BOOST_FIXTURE_TEST_CASE( f32_f64_overflow_tests, tester ) try {
   ...
   auto check = [&](const char *wast_template, const char *op, const char *param) -> bool {
      ...
      try {
         ...
         const auto& receipt = get_transaction_receipt(trx.id());
         return true;
      } catch (oac::chain::wasm_execution_error &) {
         return false;
      }
   };
   //
...
(module
  ...
  (import "env" "oac_assert" (func $oac_assert (param i32 i32)))
   (table 0 anyfunc)
   (memory $0 1)
   ...
   for( limit = start; limit < end; ++limit ) {
      ...
      try {
         ...
         BOOST_REQUIRE_EQUAL(true, chain_has_transaction(trx.id()));
         break;
      } catch (oac::chain::tx_cpu_usage_exceeded &) {
      }
  ...
  (import "env" "oac_assert" (func $oac_assert (param i32 i32)))
   ...
   while (count < 4) {
      ...
      try {
         ...
         pass = true;
         count++;
      } catch( oac::chain::leeway_deadline_exception& ) {
...
BOOST_FIXTURE_TEST_CASE( big_memory, TESTER ) try {
   ...
   string biggest_memory_wast_f = fc::format_string(biggest_memory_wast, fc::mutable_variant_object(
                                          "MAX_WASM_PAGES", oac::chain::wasm_constraints::maximum_linear_memory/(64*1024)));
   ...
   string too_big_memory_wast_f = fc::format_string(too_big_memory_wast, fc::mutable_variant_object(
                                          "MAX_WASM_PAGES_PLUS_ONE", oac::chain::wasm_constraints::maximum_linear_memory/(64*1024)+1));
   BOOST_CHECK_THROW(set_code(N(bigmem), too_big_memory_wast_f.c_str()), oac::chain::wasm_execution_error);
...
BOOST_FIXTURE_TEST_CASE( table_init_tests, TESTER ) try {
   ...
   BOOST_CHECK_THROW(set_code(N(tableinit), too_big_table), oac::chain::wasm_execution_error);
...
BOOST_FIXTURE_TEST_CASE( memory_init_border, TESTER ) try {
   ...
   BOOST_CHECK_THROW(set_code(N(memoryborder), memory_init_toolong), oac::chain::wasm_execution_error);
   BOOST_CHECK_THROW(set_code(N(memoryborder), memory_init_negative), oac::chain::wasm_execution_error);
...
BOOST_FIXTURE_TEST_CASE( lotso_globals, TESTER ) try {
   ...
   .c_str()), oac::chain::wasm_execution_error);
...
BOOST_FIXTURE_TEST_CASE( offset_check, TESTER ) try {
   ...
   for(const string& s : loadops) {
      std::stringstream ss;
      ss << "(module (memory $0 " << oac::chain::wasm_constraints::maximum_linear_memory/(64*1024) << ") (func $apply (param $0 i64) (param $1 i64) (param $2 i64)";
      ss << "(drop (" << s << " offset=" << oac::chain::wasm_constraints::maximum_linear_memory-2 << " (i32.const 0)))";
   ...
   for(const vector<string>& o : storeops) {
      std::stringstream ss;
      ss << "(module (memory $0 " << oac::chain::wasm_constraints::maximum_linear_memory/(64*1024) << ") (func $apply (param $0 i64) (param $1 i64) (param $2 i64)";
      ss << "(" << o[0] << " offset=" << oac::chain::wasm_constraints::maximum_linear_memory-2 << " (i32.const 0) (" << o[1] << ".const 0))";
      ss << ") (export \"apply\" (func $apply)) )";
   ...
   for(const string& s : loadops) {
      std::stringstream ss;
      ss << "(module (memory $0 " << oac::chain::wasm_constraints::maximum_linear_memory/(64*1024) << ") (func $apply (param $0 i64) (param $1 i64) (param $2 i64)";
      ss << "(drop (" << s << " offset=" << oac::chain::wasm_constraints::maximum_linear_memory+4 << " (i32.const 0)))";
      ss << ") (export \"apply\" (func $apply)) )";

      BOOST_CHECK_THROW(set_code(N(offsets), ss.str().c_str()), oac::chain::wasm_execution_error);
      produce_block();
   }
   for(const vector<string>& o : storeops) {
      std::stringstream ss;
      ss << "(module (memory $0 " << oac::chain::wasm_constraints::maximum_linear_memory/(64*1024) << ") (func $apply (param $0 i64) (param $1 i64) (param $2 i64)";
      ss << "(" << o[0] << " offset=" << oac::chain::wasm_constraints::maximum_linear_memory+4 << " (i32.const 0) (" << o[1] << ".const 0))";
      ss << ") (export \"apply\" (func $apply)) )";
...
// abi_serializer::to_variant failed because oac_system_abi modified via set_abi.
// This test also verifies that chain_initializer::oac_contract_abi() does not conflict
// with oac_system_abi as they are not allowed to contain duplicates.
BOOST_FIXTURE_TEST_CASE(oac_abi, TESTER) try {
   produce_blocks(2);
   ...
   // verify to_variant works on oac native contract type: newaccount
...
BOOST_FIXTURE_TEST_CASE( check_table_maximum, TESTER ) try {
   ...
   BOOST_CHECK_THROW(push_transaction(trx), oac::chain::wasm_execution_error);
   }
   ...
   signed_transaction trx;
   action act;
   act.name = oac::chain::wasm_constraints::maximum_table_elements+54334;
   act.account = N(tbl);
   act.authorization = vector<permission_level>{{N(tbl),config::active_name}};
   trx.actions.push_back(act);
      set_transaction_headers(trx);
   ...
   BOOST_CHECK_EXCEPTION(push_transaction(trx), oac::chain::wasm_execution_error, [](const oac::chain::wasm_execution_error &e) {return true;});
   }
} FC_LOG_AND_RETHROW()
...
BOOST_FIXTURE_TEST_CASE(net_usage_tests, tester ) try {
   ...
   (import "env" "oac_assert" (func $oac_assert (param i32 i32)))
      ...
      produce_blocks(1);
      signed_transaction trx;
      auto wasm = ::oac::chain::wast_to_wasm(code);
