...
BOOST_AUTO_TEST_CASE( forking ) try {
   ...
   auto r2 = c.create_accounts( {N(oac.token)} );
   wdump((fc::json::to_pretty_string(r2)));
   c.set_code( N(oac.token), oac_token_wast );
   c.set_abi( N(oac.token), oac_token_abi );
   c.produce_blocks(10);


   auto cr = c.push_action( N(oac.token), N(create), N(oac.token), mutable_variant_object()
              ("issuer",       "oac" )
   ...
   cr = c.push_action( N(oac.token), N(issue), N(oac), mutable_variant_object()
              ("to",       "dan" )
