...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   // Link authority for oac reqauth action with alice's spending key
   chain.link_authority("alice", "oac", "spending",  "reqauth");
   ...
   BOOST_CHECK_THROW( chain.link_authority("alice", "oac", "spending",  "reqauth"), action_validate_exception);

   // Unlink alice with oac reqauth
   chain.unlink_authority("alice", "oac", "reqauth");
   ...
   // Link authority for any oac action with alice's scud key
   chain.link_authority("alice", "oac", "scud");
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority("alice", "oac", "first",  "reqauth");
...
try {
   ...
   // Creating account with oac. prefix with privileged account
   chain.create_account("oac.test1");

   // Creating account with oac. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account("oac.test2", "joe"), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'oac.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( "alice", "oac", "oac.any", "reqauth" );
   chain.link_authority( "bob", "oac", "oac.any", "reqauth" );

   /// this should succeed because oac::reqauth is linked to any permission
   chain.push_reqauth("alice", { permission_level{N(alice), "spending"} }, { spending_priv_key });
...
try {
   ...
   using resource_usage_object = oac::chain::resource_limits::resource_usage_object;
   using by_owner = oac::chain::resource_limits::by_owner;
