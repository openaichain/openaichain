...
public:
   ...
   fc::variant get_global_state() {
      vector<char> data = get_row_by_account( N(oac), N(oac), N(global), N(global) );
      if (data.empty()) std::cout << "\nData is empty\n" << std::endl;
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "oac_global_state", data );

   }

    auto buyram( name payer, name receiver, asset ram ) {
       auto r = base_tester::push_action(N(oac), N(buyram), payer, mvo()
                    ("payer", payer)
                    ("receiver", receiver)
                    ("quant", ram)
                    );
    ...
    auto delegate_bandwidth( name from, name receiver, asset net, asset cpu, uint8_t transfer = 1) {
       auto r = base_tester::push_action(N(oac), N(delegatebw), from, mvo()
                    ("from", from )
                    ("receiver", receiver)
    ...
    auto register_producer(name producer) {
       auto r = base_tester::push_action(N(oac), N(regproducer), producer, mvo()
    ...
    auto undelegate_bandwidth( name from, name receiver, asset net, asset cpu ) {
       auto r = base_tester::push_action(N(oac), N(undelegatebw), from, mvo()
                    ("from", from )
                    ("receiver", receiver)
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance(N(oac.token), symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // Create oac.msig and oac.token
        create_accounts({N(oac.msig), N(oac.token), N(oac.ram), N(oac.ramfee), N(oac.stake), N(oac.vpay), N(oac.bpay), N(oac.saving) });

        // Set code for the following accounts:
        //  - oac (code: oac.bios) (already set by tester constructor)
        //  - oac.msig (code: oac.msig)
        //  - oac.token (code: oac.token)
        set_code_abi(N(oac.msig), oac_msig_wast, oac_msig_abi);//, &oac_active_pk);
        set_code_abi(N(oac.token), oac_token_wast, oac_token_abi); //, &oac_active_pk);

        // Set privileged for oac.msig and oac.token
        set_privileged(N(oac.msig));
        set_privileged(N(oac.token));

        // Verify oac.msig and oac.token is privileged
        const auto& oac_msig_acc = get<account_object, by_name>(N(oac.msig));
        BOOST_TEST(oac_msig_acc.privileged == true);
        const auto& oac_token_acc = get<account_object, by_name>(N(oac.token));
        BOOST_TEST(oac_token_acc.privileged == true);


        // Create SYS tokens in oac.token, set its manager as oac
        ...
        create_currency(N(oac.token), config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SYS tokens to oac.system
        issue(N(oac.token), config::system_account_name, config::system_account_name, initial_supply);
        ...
        for( const auto& a : test_genesis ) {
           create_account( a.aname, N(oac) );
        }

        // Set oac.system to oac
        set_code_abi(N(oac), oac_system_wast, oac_system_abi);
        ...
        for( const auto& a : test_genesis ) {
           ...
           auto r = buyram(N(oac), a.aname, asset(ram));
           BOOST_REQUIRE( !r->except_ptr );

           r = delegate_bandwidth(N(oac.stake), a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        auto votepro = [&]( account_name voter, vector<account_name> producers ) {
          ...
          base_tester::push_action(N(oac), N(voteproducer), voter, mvo()
                                ("voter",  name(voter))
                                ("proxy", name(0) )
                                ("producers", producers)
                     );
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == "oac");
        ...
        BOOST_REQUIRE_THROW(claim_rewards(N(runnerup1)), oac_assert_message_exception);
        ...
        BOOST_REQUIRE_THROW(undelegate_bandwidth(N(b1), N(b1), core_from_string("49999500.0000"), core_from_string("49999500.0000")), oac_assert_message_exception);
