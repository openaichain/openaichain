...
namespace oac_system {

class oac_system_tester : public TESTER {
public:

   oac_system_tester()
   : oac_system_tester([](TESTER& ) {}){}

   template<typename Lambda>
   oac_system_tester(Lambda setup) {
      setup(*this);

      produce_blocks( 2 );

      create_accounts({ N(oac.token), N(oac.ram), N(oac.ramfee), N(oac.stake),
               N(oac.bpay), N(oac.vpay), N(oac.saving), N(oac.names) });
      ...
      {
         const auto& accnt = control->db().get<account_object,by_name>( N(oac.token) );
         abi_def abi;
      ...
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance("oac")  + get_balance("oac.ramfee") + get_balance("oac.stake") + get_balance("oac.ram") );
   }
   ...
   action_result buyram( const account_name& payer, account_name receiver, const asset& oacin ) {
      return push_action( payer, N(buyram), mvo()( "payer",payer)("receiver",receiver)("quant",oacin) );
   }
   ...
   asset get_balance( const account_name& act ) {
      vector<char> data = get_row_by_account( N(oac.token), act, N(accounts), symbol(CORE_SYMBOL).to_symbol_code().value );
   ...
   fc::variant get_stats( const string& symbolname ) {
      auto symb = oac::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( N(oac.token), symbol_code, N(stat), symbol_code );
   ...
   abi_serializer initialize_multisig() {
      ...
      {
         create_account_with_resources( N(oac.msig), config::system_account_name );
         BOOST_REQUIRE_EQUAL( success(), buyram( "oac", "oac.msig", core_from_string("5000.0000") ) );
         produce_block();
   ...
   */
   ...
   vector<name> active_and_vote_producers() {
      ...
      transfer( "oac", "alice1111111", core_from_string("650000000.0000"), "oac" );
      ...
      auto trace_auth = TESTER::push_action(config::system_account_name, updateauth::get_name(), config::system_account_name, mvo()
                                            ...
                                            ("auth",  authority(1, {key_weight{get_public_key( config::system_account_name, "active" ), 1}}, {
                                                  permission_level_weight{{config::system_account_name, config::oac_code_name}, 1},
...
inline uint64_t M( const string& oac_str ) {
   return core_from_string( oac_str ).get_amount();
}
