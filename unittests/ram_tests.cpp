 ...
 * ram_tests test case
 *************************************************************************************/
BOOST_FIXTURE_TEST_CASE(ram_tests, oac_system::oac_system_tester) { try {
   ...
   buyrambytes(N(oac), N(oac), 70000);
   produce_blocks(10);
   create_account_with_resources(N(testram11111),N(oac), init_request_bytes);
   create_account_with_resources(N(testram22222),N(oac), init_request_bytes + 1150);
   produce_blocks(10);
   BOOST_REQUIRE_EQUAL( success(), stake( "oac.stake", "testram11111", core_from_string("10.0000"), core_from_string("5.0000") ) );
   produce_blocks(10);
   ...
   for (auto i = 0; i < 10; ++i) {
      try {
         ...
         buyrambytes(N(oac), N(testram11111), increment_contract_bytes);
         buyrambytes(N(oac), N(testram22222), increment_contract_bytes);
      }
   }
   produce_blocks(10);
   ...
   buyrambytes(N(oac), N(testram11111), more_ram);
   buyrambytes(N(oac), N(testram22222), more_ram);
