...
FC_REFLECT(account_t, (owner)(oac_balance)(open_offers)(open_games));
...
BOOST_FIXTURE_TEST_CASE( dice_test, dice_tester ) try {
   ...
   create_accounts( {N(oac.token), N(dice),N(alice),N(bob),N(carol),N(david)}, false);
   ...
   push_action(N(oac.token), N(create), N(oac.token), mvo()
     ("issuer", "oac.token")
     ("maximum_supply", core_from_string("1000000000.0000"))
   );

   push_action(N(oac.token), N(issue), N(oac.token), mvo()
     ("to", "oac")
     ("quantity", core_from_string("1000000000.0000"))
     ("memo", "")
   );

   transfer( N(oac), N(alice), core_from_string("10000.0000"), "", N(oac.token) );
   transfer( N(oac), N(bob),   core_from_string("10000.0000"), "", N(oac.token) );
   transfer( N(oac), N(carol), core_from_string("10000.0000"), "", N(oac.token) );
   ...
   BOOST_REQUIRE_EQUAL(
      get_currency_balance(N(oac.token), symbol(CORE_SYMBOL), N(alice)),
