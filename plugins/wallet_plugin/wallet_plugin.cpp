 ...
 */
#include <oac/wallet_plugin/wallet_plugin.hpp>
...
void wallet_plugin::set_program_options(options_description& cli, options_description& cfg) {
   cfg.add_options()
         ...
         ("oac-key", bpo::value<std::string>(),
          "oac key that will be imported automatically when a wallet is created.")
         ;
}
...
void wallet_plugin::plugin_initialize(const variables_map& options) {
   ...
   if (options.count("oac-key")) {
      std::string oac_wif_key = options.at("oac-key").as<std::string>();
      wallet_manager_ptr->set_oac_key(oac_wif_key);
   }
}
} // namespace oac
