...
try:
    ...
    # create accounts via oac as otherwise a bid is needed 
    Print("Create new account %s via %s" % (testeraAccount.name, cluster.oacAccount.name))
    transId=node.createInitializeAccount(testeraAccount, cluster.oacAccount, stakedDeposit=0, waitForTransBlock=False)
    if transId is None:
        cmdError("%s create account" % (testeraAccount.name))
        errorExit("Failed to create account %s" % (testeraAccount.name))

    Print("Create new account %s via %s" % (currencyAccount.name, cluster.oacAccount.name))
    transId=node.createInitializeAccount(currencyAccount, cluster.oacAccount, stakedDeposit=5000)
    if transId is None:
        cmdError("%s create account" % (ClientName))
        errorExit("Failed to create account %s" % (currencyAccount.name))

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.oacAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.oacAccount, waitForTransBlock=True)
    if transId is None:
        cmdError("%s create account" % (ClientName))
    ...
    contractDir="contracts/oac.token"
    wastFile="contracts/oac.token/oac.token.wast"
    abiFile="contracts/oac.token/oac.token.abi"
    ...
    if not enableMongo:
        ...
        if abiName != "transfer" or abiActionName != "transfer" or abiType != "transfer":
            errorExit("FAILURE - get OAC account failed", raw=True)
