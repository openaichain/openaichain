...
my $oac_home = defined $ENV{OAC_HOME} ? $ENV{OAC_HOME} : getcwd;
my $oacd = $oac_home . "/programs/oacd/oacd";
my $oacc = $oac_home . "/programs/oacc/oacc";

my $nodes = defined $ENV{OAC_TEST_RING} ? $ENV{OAC_TEST_RING} : "1";
my $pnodes = defined $ENV{OAC_TEST_PRODUCERS} ? $ENV{OAC_TEST_PRODUCERS} : "1";

my $prods = 21;
my $genesis = "$oac_home/genesis.json";
...
if (!GetOptions("nodes=i" => \$nodes,
    ...
    print "where:\n";
    print "--nodes=n (default = 1) sets the number of oacd instances to launch\n";
...
sub write_config {
    ...
    if (defined $producer) {
        ...
        print $cfg "private-key = [\"OAC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV\",\"5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3\"]\n";

        print $cfg "plugin = oac::producer_plugin\n";
        print $cfg "plugin = oac::chain_api_plugin\n";
...
sub perform_work {
    ...
    if ($mode == 0) {
        ...
        my $counter = 0;
        while (time () < $stoptime) {
            `$oacc transfer oac inita 10 >> oacc.out 2>> oacc.err`;
            $counter++;
            if ($counter % 1000 == 0) {
