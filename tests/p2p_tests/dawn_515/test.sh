...
read -d '' genesis << EOF
{
  ...
  "initial_key": "OAC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV",
...
plugin = oac::producer_plugin
plugin = oac::chain_api_plugin
plugin = oac::net_plugin
plugin = oac::history_api_plugin
http-server-address = 127.0.0.1:8888
blocks-dir = blocks
p2p-listen-endpoint = 0.0.0.0:9876
allowed-connection = any
private-key = ['OAC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV','5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3']
...
programs/oac-launcher/oac-launcher -p $pnodes -n $total_nodes --nogen -d $delay
...
b5idbios=`./programs/cloac/cloac -u http://localhost:8888 get block 5 | grep "^ *\"id\""`
b5id00=`./programs/cloac/cloac -u http://localhost:8889 get block 5 | grep "^ *\"id\""`
b5id01=`./programs/cloac/cloac -u http://localhost:8890 get block 5 | grep "^ *\"id\""`
