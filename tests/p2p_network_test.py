...
# oac should have the same key as defproducera
oac = copy.copy(defproduceraAccount)
oac.name = "oac"
...
wastFile="contracts/oac.system/oac.system.wast"
abiFile="contracts/oac.system/oac.system.abi"
Print("\nPush system contract %s %s" % (wastFile, abiFile))
trans=node0.publishContract(oac.name, wastFile, abiFile, waitForTransBlock=True)
if trans is None:
    Utils.errorExit("Failed to publish oac.system.")
else:
...
try:
    ...
    for cmdInd in range(maxIndex):
        (transIdList, checkacct, expBal, errmsg) = module.execute(cmdInd, node0, testeraAccount, oac)
