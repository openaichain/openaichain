...
class Utils:
    ...
    EosClientPath="programs/cloac/cloac"

    EosWalletName="koacd"
    EosWalletPath="programs/koacd/"+ EosWalletName

    EosServerName="nodoac"
    EosServerPath="programs/nodoac/"+ EosServerName

    EosLauncherPath="programs/oac-launcher/oac-launcher"
    MongoPath="mongo"
    ...
    # mongoSyncTime: nodoac mongodb plugin seems to sync with a 10-15 seconds delay. This will inject
...
class Node(object):
    ...
    def __init__(self, host, port, pid=None, cmd=None, enableMongo=False, mongoHost="localhost", mongoPort=27017, mongoDb="OACtest"):
        self.host=host
        self.port=port
        self.pid=pid
        self.cmd=cmd
    ...
    # Get actions mapped to an account (cloac get actions)
    ...
    def getAccountEosBalanceStr(self, scope):
        """Returns SYS currency0000 account balance from cloac get table command. Returned balance is string following syntax "98.0311 SYS". """
        assert isinstance(scope, str)
        if not self.enableMongo:
            amount=self.getTableAccountBalance("oac.token", scope)
            ...
            if account is not None:
                balance=account["oac_balance"]
                return balance

        return None

    def getAccountEosBalance(self, scope):
        """Returns SYS currency0000 account balance from cloac get table command. Returned balance is an integer e.g. 980311. """
    ...
    def getHeadBlockNum(self):
        """returns head block number(string) as returned by cloac get info."""
        if not self.enableMongo:
            info=self.getInfo()
            if info is not None:
...
class WalletMgr(object):
    __walletLogFile="test_koacd_output.log"
    __walletDataDir="test_wallet_0"

    # pylint: disable=too-many-arguments
    # walletd [True|False] True=Launch wallet(koacd) process; False=Manage launch process externally.
    def __init__(self, walletd, nodoacPort=8888, nodoacHost="localhost", port=8899, host="localhost"):
        self.walletd=walletd
        self.nodoacPort=nodoacPort
        self.nodoacHost=nodoacHost
        self.port=port
        self.host=host
        self.wallets={}
        self.__walletPid=None
        self.endpointArgs="--url http://%s:%d" % (self.nodoacHost, self.nodoacPort)
    ...
    def launch(self):
        if not self.walletd:
            Utils.Print("ERROR: Wallet Manager wasn't configured to launch koacd")
            return False
    ...
    def killall(self, allInstances=False):
        """Kill koac instances. allInstances will kill all koac instances running on the system."""
        if self.__walletPid:
...
class Cluster(object):
    ...
    # pylint: disable=too-many-arguments
    # walletd [True|False] Is koacd running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=8899, enableMongo=False, mongoHost="localhost", mongoPort=27017, mongoDb="OACtest", defproduceraPrvtKey=None, defproducerbPrvtKey=None, staging=False):
        """Cluster container.
        walletd [True|False] Is wallet koacd running. If not load the wallet plugin
        localCluster [True|False] Is cluster local to host.
        host: oac server host
        port: oac server port
        walletHost: oac wallet host
        walletPort: wos wallet port
        enableMongo: Include mongoDb support, configures oac mongo plugin
    ...
    def launch(self, pnodes=1, totalNodes=1, prodCount=1, topo="mesh", p2pPlugin="net", delay=1, onlyBios=False, dontKill=False, dontBootstrap=False):
        ...
        nodoacArgs="--max-transaction-time 5000 --filter-on * --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            nodoacArgs += " --plugin oac::wallet_api_plugin"
        if self.enableMongo:
            nodoacArgs += " --plugin oac::mongo_db_plugin --delete-all-blocks --mongodb-uri %s" % self.mongoUri

        if nodoacArgs:
            cmdArr.append("--nodoac")
            cmdArr.append(nodoacArgs)
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        fromm=source
        to=accounts[0]
        Utils.Print("Transfer %s units from account %s to %s on oac server port %d" % (
    ...
    def validateSpreadFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        for node in self.nodes:
            ...
            if node.validateFunds(initialBalances, transferAmount, source, accounts) is False:
                Utils.Print("ERROR: Failed to validate funds on oac node port: %d" % (node.port))
                return False
    ...
    def validateAccounts(self, accounts, testSysAccounts=True):
        ...
        myAccounts = []
        if testSysAccounts:
            myAccounts += [self.oacAccount, self.defproduceraAccount, self.defproducerbAccount]
        if accounts:
            assert(isinstance(accounts, list))
            myAccounts += accounts
    ...
    def bootstrap(totalNodes, prodCount, biosHost, biosPort, dontKill=False, onlyBios=False):
        ...
        try:
            ...
            oacName="oac"
            oacKeys=producerKeys[oacName]
            oacAccount=Account(oacName)
            oacAccount.ownerPrivateKey=oacKeys["private"]
            oacAccount.ownerPublicKey=oacKeys["public"]
            oacAccount.activePrivateKey=oacKeys["private"]
            oacAccount.activePublicKey=oacKeys["public"]

            if not walletMgr.importKey(oacAccount, ignWallet):
                Utils.Print("ERROR: Failed to import %s account keys into ignition wallet." % (oacName))
                return False
            ...
            trans=biosNode.publishContract(oacAccount.name, contractDir, wastFile, abiFile, waitForTransBlock=True)
            if trans is None:
            ...
            for name, keys in producerKeys.items():
                ...
                trans=biosNode.createAccount(initx, oacAccount, 0)
                if trans is None:
            ...
            if not onlyBios:
                if prodCount == -1:
                    ...
                    with open(setProdsFile, "r") as f:
                        ...
                        opts="--permission oac@active"
                        myTrans=biosNode.pushMessage("oac", "setprods", setProdsStr, opts)
                    ...
                    trans=biosNode.pushMessage("oac", "setprods", setProdsStr, opts)
                ...
                # wait for block production handover (essentially a block produced by anyone but oac).
                lam = lambda: biosNode.getInfo()["head_block_producer"] != "oac"
                ret=Utils.waitForBool(lam)
                if not ret:
                    Utils.Print("ERROR: Block production handover failed.")
                    return False

            oacTokenAccount=copy.deepcopy(oacAccount)
            oacTokenAccount.name="oac.token"
            trans=biosNode.createAccount(oacTokenAccount, oacAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (oacTokenAccount.name))
                return False

            oacRamAccount=copy.deepcopy(oacAccount)
            oacRamAccount.name="oac.ram"
            trans=biosNode.createAccount(oacRamAccount, oacAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (oacRamAccount.name))
                return False

            oacRamfeeAccount=copy.deepcopy(oacAccount)
            oacRamfeeAccount.name="oac.ramfee"
            trans=biosNode.createAccount(oacRamfeeAccount, oacAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (oacRamfeeAccount.name))
                return False

            oacStakeAccount=copy.deepcopy(oacAccount)
            oacStakeAccount.name="oac.stake"
            trans=biosNode.createAccount(oacStakeAccount, oacAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (oacStakeAccount.name))
                return False
            ...
            abiFile="contracts/%s/%s.abi" % (contract, contract)
            Utils.Print("Publish %s contract" % (contract))
            trans=biosNode.publishContract(oacTokenAccount.name, contractDir, wastFile, abiFile, waitForTransBlock=True)
            if trans is None:
                Utils.Print("ERROR: Failed to publish contract %s." % (contract))
                return False

            # Create currency0000, followed by issue currency0000
            contract=oacTokenAccount.name
            Utils.Print("push create action to %s contract" % (contract))
            action="create"
            data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\",\"can_freeze\":\"0\",\"can_recall\":\"0\",\"can_whitelist\":\"0\"}" % (oacTokenAccount.name, CORE_SYMBOL)
            opts="--permission %s@active" % (contract)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to push create action to oac contract.")
                return False
            ...
            action="issue"
            data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (oacAccount.name, CORE_SYMBOL)
            opts="--permission %s@active" % (contract)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to push issue action to oac contract.")
                return False
            ...
            Utils.Print("Verify oac issue, Expected: %s" % (expectedAmount))
            actualAmount=biosNode.getAccountEosBalanceStr(oacAccount.name)
            ...
            action="transfer"
            for name, keys in producerKeys.items():
                data="{\"from\":\"%s\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (oacAccount.name, name, initialFunds, "init transfer")
                opts="--permission %s@active" % (oacAccount.name)
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (oacTokenAccount.name, name))
                    return False
    ...
    # Kills a percentange of Eos instances starting from the tail and update oacInstanceInfos state
    ...
    def dumpErrorDetails(self):
        fileName="etc/oac/node_bios/config.ini"
        Cluster.dumpErrorDetailImpl(fileName)
        fileName="var/lib/node_bios/stderr.txt"
        Cluster.dumpErrorDetailImpl(fileName)

        for i in range(0, len(self.nodes)):
            fileName="etc/oac/node_%02d/config.ini" % (i)
            Cluster.dumpErrorDetailImpl(fileName)
            fileName="var/lib/node_%02d/stderr.txt" % (i)
            Cluster.dumpErrorDetailImpl(fileName)

    def killall(self, silent=True, allInstances=False):
        """Kill cluster nodoac instances. allInstances will kill all nodoac instances running on the system."""
        cmd="%s -k 9" % (Utils.EosLauncherPath)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            if not silent: Utils.Print("Launcher failed to shut down oac cluster.")

        if allInstances:
            # ocassionally the launcher cannot kill the oac server
            cmd="pkill -9 %s" % (Utils.EosServerName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
                if not silent: Utils.Print("Failed to shut down oac cluster.")
    ...
    def cleanup(self):
        for f in glob.glob("var/lib/node_*"):
            shutil.rmtree(f)
        for f in glob.glob("etc/oac/node_*"):
            shutil.rmtree(f)
