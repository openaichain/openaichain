...
plugin = oac::account_history_plugin
plugin = oac::account_history_api_plugin"""
...
def stageScenario(stagedNodeInfos):
    ...
    for stagedNodeInfo in stagedNodeInfos:
        configPath=os.path.join(stagingDir, "etc/oac/node_%02d" % (count))
...
def myTest(transWillEnterBlock):
    ...
    try:
        ...
        if cluster.launch(pnodes, total_nodes, topo, delay) is False:
            error("Failed to stand up oac cluster.")
            return False
        ...
        if walletMgr.launch() is False:
            error("Failed to stand up oac walletd.")
            return False
