...
killAll()
{
  programs/oac-launcher/oac-launcher -k 15
}
...
getHeadBlockNum()
{
  INFO="$(programs/cloac/cloac get info)"
  verifyErrorCode "cloac get info"
...
# stand up nodoac cluster
launcherOpts="-p $pnodes -n $total_nodes -s $topo -d $delay"
echo Launcher options: --nodoac \"--plugin oac::wallet_api_plugin\" $launcherOpts
programs/oac-launcher/oac-launcher --nodoac "--plugin oac::wallet_api_plugin" $launcherOpts
sleep 7
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/cloac/cloac --port $port2 get block 1)"
    verifyErrorCode "cloac get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/cloac/cloac create key)"
verifyErrorCode "cloac create key"
...
PASSWORD_INITA="$(programs/cloac/cloac wallet create --name inita)"
verifyErrorCode "cloac wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
programs/cloac/cloac wallet import --name inita $INITA_PRV_KEY
verifyErrorCode "cloac wallet import"
programs/cloac/cloac wallet import --name inita $PRV_KEY1
verifyErrorCode "cloac wallet import"
programs/cloac/cloac wallet import --name inita $PRV_KEY2
verifyErrorCode "cloac wallet import"
programs/cloac/cloac wallet import --name inita $PRV_KEY3
verifyErrorCode "cloac wallet import"
...
ACCOUNT_INFO="$(programs/cloac/cloac create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "cloac create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/cloac/cloac get account testera)"
verifyErrorCode "cloac get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/cloac/cloac transfer inita testera 975321 "test transfer")"
    verifyErrorCode "cloac transfer"
   ...
   TRANS_INFO="$(programs/cloac/cloac --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "cloac get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done
