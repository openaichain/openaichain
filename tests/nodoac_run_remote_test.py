...
###############################################################
# nodoac_run_remote_test
#  Tests remote capability of the nodoac_run_test. Test will setup cluster and pass nodes info to nodoac_run_test. E.g.
#  nodoac_run_remote_test.py -v --clean-run --dump-error-detail
###############################################################
...
pnodes=1
total_nodes=pnodes
actualTest="tests/nodoac_run_test.py"
...
try:
    ...
    Print("Starting up %s test: %s" % ("nodoac", actualTest))
    Print("cmd: %s\n" % (cmd))
