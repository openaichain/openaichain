...
###############################################################
# Test for validating the dirty db flag sticks repeated nodoac restart attempts
###############################################################
...
def runNodoacAndGetOutput(myNodeId, myTimeout=3):
    """Startup nodoac, wait for timeout (before forced shutdown) and collect output. Stdout, stderr and return code are returned in a dictionary."""
    Print("Launching nodoac process id: %d" % (myNodeId))
    cmd="programs/nodoac/nodoac --config-dir etc/oac/node_bios --data-dir var/lib/node_bios"
    Print("cmd: %s" % (cmd))
    ...
    try:
        ...
        Print("ERROR: Nodoac is running beyond the defined wait time. Hard killing nodoac instance.")
...
try:
    ...
    Print("Restart nodoac repeatedly to ensure dirty database flag sticks.")
    nodeId=0
    timeout=3
    
    for i in range(0,3):
        Print("Attempt %d." % (i))
        ret = runNodoacAndGetOutput(nodeId, timeout)
        assert(ret)
