...
{
   try {
      app().set_version(oac::nodoac::config::version);
      app().register_plugin<history_plugin>();

      auto root = fc::app_path();
      app().set_default_data_dir(root / "oac/nodoac/data" );
      app().set_default_config_dir(root / "oac/nodoac/config" );
      if(!app().initialize<chain_plugin, http_plugin, net_plugin, producer_plugin>(argc, argv))
         return INITIALIZE_FAIL;
      initialize_logging();
      ilog("nodoac version ${ver}", ("ver", oac::utilities::common::itoh(static_cast<uint32_t>(app().version()))));
      ilog("oac root is ${root}", ("root", root.string()));
      app().startup();
      app().exec();
