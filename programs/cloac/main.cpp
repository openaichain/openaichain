 ...
 *  @defgroup oacclienttool OACIO Command Line Client Reference
 *  @brief Tool for sending transactions and querying state from @ref nodoac
 *  @ingroup oacclienttool
 */

/**
  @defgroup oacclienttool

  @section intro Introduction to cloac

  `cloac` is a command line tool that interfaces with the REST api exposed by @ref nodoac. In order to use `cloac` you will need to
  have a local copy of `nodoac` running and configured to load the 'oac::chain_api_plugin'.

   cloac contains documentation for all of its commands. For a list of all commands known to cloac, simply run it with no arguments:
```
$ ./cloac
Command Line Interface to OACIO Client
Usage: programs/cloac/cloac [OPTIONS] SUBCOMMAND

Options:
  -h,--help                   Print this help message and exit
  -u,--url TEXT=http://localhost:8888/
                              the http/https URL where nodoac is running
  --wallet-url TEXT=http://localhost:8888/
                              the http/https URL where koacd is running
...
Subcommands:
  ...
  transfer                    Transfer OAC from account to account
...
Usage: ./cloac create SUBCOMMAND

Subcommands:
  key                         Create a new keypair and print the public and private keys
  account                     Create a new account on the blockchain

$ ./cloac create account
Create a new account on the blockchain
Usage: ./cloac create account [OPTIONS] creator name OwnerKey ActiveKey
...
#include <oac/chain/name.hpp>
#include <oac/chain/config.hpp>
#include <oac/chain/wast_to_wasm.hpp>
#include <oac/chain/trace.hpp>
...
using namespace oac::utilities;
using namespace oac::client::help;
using namespace oac::client::http;
using namespace oac::client::localize;
using namespace oac::client::config;
using namespace boost::filesystem;

FC_DECLARE_EXCEPTION( explained_exception, 9000000, "explained exception, see error log" );
FC_DECLARE_EXCEPTION( localized_exception, 10000000, "an error occured" );
#define OACC_ASSERT( TEST, ... ) \
  FC_EXPAND_MACRO( \
...
fc::variant call( const std::string& url,
                  ...
                  const T& v ) {
   try {
      oac::client::http::connection_param *cp = new oac::client::http::connection_param((std::string&)url, (std::string&)path,
              no_verify ? false : true, headers);

      return oac::client::http::do_http_call( *cp, fc::variant(v), print_request, print_response );
   }
   catch(boost::system::system_error& e) {
      if(url == ::url)
         std::cerr << localized("Failed to connect to nodoac at ${u}; is nodoac running?", ("u", url)) << std::endl;
      else if(url == ::wallet_url)
         std::cerr << localized("Failed to connect to koacd at ${u}; is koacd running?", ("u", url)) << std::endl;
...
oac::chain_apis::read_only::get_info_results get_info() {
   return call(url, get_info_func).as<oac::chain_apis::read_only::get_info_results>();
}
...
fc::variant push_transaction( signed_transaction& trx, int32_t extra_kcpu = 1000, packed_transaction::compression_type compression = packed_transaction::none ) {
   ...
   } OAC_RETHROW_EXCEPTIONS(invalid_ref_block_exception, "Invalid reference block num or id: ${block_num_or_id}", ("block_num_or_id", tx_ref_block_num_or_id));
...
chain::action create_newaccount(const name& creator, const name& newaccount, public_key_type owner, public_key_type active) {
   return action {
      ...
      oac::chain::newaccount{
         .creator      = creator,
         .name         = newaccount,
         .owner        = oac::chain::authority{1, {{owner, 1}}, {}},
         .active       = oac::chain::authority{1, {{active, 1}}, {}}
      }
   };
}
...
authority parse_json_authority(const std::string& authorityJsonOrFile) {
   ...
   } OAC_RETHROW_EXCEPTIONS(authority_type_exception, "Fail to parse Authority JSON '${data}'", ("data",authorityJsonOrFile))
}

authority parse_json_authority_or_key(const std::string& authorityJsonOrFile) {
   if (boost::istarts_with(authorityJsonOrFile, "OAC")) {
      try {
         return authority(public_key_type(authorityJsonOrFile));
      } OAC_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", authorityJsonOrFile))
   } else {
...
asset to_asset( const string& code, const string& s ) {
   static map<oac::chain::symbol_code, oac::chain::symbol> cache;
   auto a = asset::from_string( s );
   oac::chain::symbol_code sym = a.get_symbol().to_symbol_code();
   ...
   if ( it == cache.end() ) {
      ...
      if (obj_it != obj.end()) {
         auto result = obj_it->value().as<oac::chain_apis::read_only::get_currency_stats_result>();
...
void try_port( uint16_t port, uint32_t duration ) {
   ...
   while ( !port_used(port)) {
      if (duration_cast<std::chrono::milliseconds>( system_clock::now().time_since_epoch()).count() - start_time > duration ) {
         std::cerr << "Unable to connect to koacd, if koacd is running please kill the process and try again.\n";
         throw connection_exception(fc::log_messages{FC_LOG_MESSAGE(error, "Unable to connect to koacd")});
      }
   }
}

void ensure_koacd_running(CLI::App* app) {
    // get, version, net do not require koacd
    ...
    if (port_used(uint16_t(wallet_port)))  // Hopefully taken by koacd
        return;


    boost::filesystem::path binPath = boost::dll::program_location();
    binPath.remove_filename();
    // This extra check is necessary when running cloac like this: ./cloac ...
    if (binPath.filename_is_dot())
        binPath.remove_filename();
    binPath.append("koacd"); // if cloac and koacd are in the same installation directory
    if (!boost::filesystem::exists(binPath)) {
        binPath.remove_filename().remove_filename().append("koacd").append("koacd");
    }
    ...
    if (boost::filesystem::exists(binPath)) {
        ...
        ::boost::process::child koac(binPath, pargs,
                                     bp::std_in.close(),
                                     bp::std_out > bp::null,
                                     bp::std_err > bp::null);
        if (koac.running()) {
        ...
        std::cerr << "No wallet service listening on 127.0.0.1: " << std::to_string(wallet_port)
                  << ". Cannot automatically start koacd because koacd was not found." << std::endl;
    }
}
...
struct register_producer_subcommand {
   ...
   register_producer_subcommand(CLI::App* actionRoot) {
      ...
      register_producer->set_callback([this] {
         ...
         } OAC_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid producer public key: ${public_key}", ("public_key", producer_key_str))
   ...
   create_account_subcommand(CLI::App* actionRoot, bool s) : simple(s) {
      ...
      if (!simple) {
         createAccount->add_option("--stake-net", stake_net,
                                   (localized("The amount of OAC delegated for net bandwidth")))->required();
         createAccount->add_option("--stake-cpu", stake_cpu,
                                   (localized("The amount of OAC delegated for CPU bandwidth")))->required();
         ...
         createAccount->add_option("--buy-ram", buy_ram_oac,
                                   (localized("The amount of RAM bytes to purchase for the new account in OAC")));
         createAccount->add_flag("--transfer", transfer,
                                 (localized("Transfer voting power and right to unstake OAC to receiver")));
      }
      ...
      createAccount->set_callback([this] {
            ...
            } OAC_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid owner public key: ${public_key}", ("public_key", owner_key_str));
            try {
               active_key = public_key_type(active_key_str);
            } OAC_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid active public key: ${public_key}", ("public_key", active_key_str));
            auto create = create_newaccount(creator, account_name, owner_key, active_key);
            if (!simple) {
               if ( buy_ram_oac.empty() && buy_ram_bytes_in_kbytes == 0) {
                  std::cerr << "ERROR: Either --buy-ram or --buy-ram-kbytes with non-zero value is required" << std::endl;
                  return;
               }
               action buyram = !buy_ram_oac.empty() ? create_buyram(creator, account_name, to_asset(buy_ram_oac))
...
struct vote_producers_subcommand {
   string voter_str;
   vector<oac::name> producer_names;
...
struct approve_producer_subcommand {
   ...
   approve_producer_subcommand(CLI::App* actionRoot) {
      ...
      approve_producer->set_callback([this] {
            ...
            auto res = result.as<oac::chain_apis::read_only::get_table_rows_result>();
...
struct list_producers_subcommand {
   ...
   list_producers_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->set_callback([this] {
         ...
         auto result = rawResult.as<oac::chain_apis::read_only::get_producers_result>();
...
struct delegate_bandwidth_subcommand {
   ...
   delegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      delegate_bandwidth->add_option("stake_net_quantity", stake_net_amount, localized("The amount of OAC to stake for network bandwidth"))->required();
      delegate_bandwidth->add_option("stake_cpu_quantity", stake_cpu_amount, localized("The amount of OAC to stake for CPU bandwidth"))->required();
      delegate_bandwidth->add_flag("--transfer", transfer, localized("Transfer voting power and right to unstake OAC to receiver"));
...
struct undelegate_bandwidth_subcommand {
   ...
   undelegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      undelegate_bandwidth->add_option("unstake_net_quantity", unstake_net_amount, localized("The amount of OAC to undelegate for network bandwidth"))->required();
      undelegate_bandwidth->add_option("unstake_cpu_quantity", unstake_cpu_amount, localized("The amount of OAC to undelegate for CPU bandwidth"))->required();
...
struct bidname_subcommand {
   ...
   bidname_subcommand(CLI::App *actionRoot) {
      ...
      bidname->add_option("bid", bid_amount, localized("The amount of OAC to bid"))->required();
   ...
   bidname_info_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->set_callback([this] {
         auto rawResult = call(get_table_func, fc::mutable_variant_object("json", true)
                               ("code", "oac")("scope", "oac")("table", "namebids")
                               ("lower_bound", oac::chain::string_to_name(newname_str.c_str()))("limit", 1));
         if ( print_json ) {
            std::cout << fc::json::to_pretty_string(rawResult) << std::endl;
            return;
         }
         auto result = rawResult.as<oac::chain_apis::read_only::get_table_rows_result>();
         if ( result.rows.empty() ) {
...
struct buyram_subcommand {
   ...
   buyram_subcommand(CLI::App* actionRoot) {
      ...
      buyram->add_option("tokens", amount, localized("The amount of OAC to pay for RAM"))->required();
...
struct sellram_subcommand {
   ...
   sellram_subcommand(CLI::App* actionRoot) {
      ...
      sellram->add_option("account", receiver_str, localized("The account to receive OAC for sold RAM"))->required();
...
void get_account( const string& accountName, bool json_format ) {
   ...
   auto res = json.as<oac::chain_apis::read_only::get_account_results>();
   ...
   if (!json_format) {
      ...
      unordered_map<name, oac::chain_apis::permission> cache;
...
int main( int argc, char** argv ) {
   ...
   CLI::App app{"Command Line Interface to OACIO Client"};
   app.require_subcommand();
   app.add_option( "-H,--host", obsoleted_option_host_port, localized("the host where nodoac is running") )->group("hidden");
   app.add_option( "-p,--port", obsoleted_option_host_port, localized("the port where nodoac is running") )->group("hidden");
   app.add_option( "--wallet-host", obsoleted_option_host_port, localized("the host where koacd is running") )->group("hidden");
   app.add_option( "--wallet-port", obsoleted_option_host_port, localized("the port where koacd is running") )->group("hidden");

   app.add_option( "-u,--url", url, localized("the http/https URL where nodoac is running"), true );
   app.add_option( "--wallet-url", wallet_url, localized("the http/https URL where koacd is running"), true );

   app.add_option( "-r,--header", header_opt_callback, localized("pass specific HTTP header; repeat this option to pass multiple headers"));
   app.add_flag( "-n,--no-verify", no_verify, localized("don't verify peer certificate when using HTTPS"));
   app.set_callback([&app]{ ensure_koacd_running(&app);});
   ...
   version->add_subcommand("client", localized("Retrieve version information of the client"))->set_callback([] {
     std::cout << localized("Build version: ${ver}", ("ver", oac::client::config::version_str)) << std::endl;
   });
   ...
   getAccounts->set_callback([&] {
      ...
      } OAC_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", public_key_str))
   ...
   getTransaction->set_callback([&] {
      ...
      } OAC_RETHROW_EXCEPTIONS(transaction_id_type_exception, "Invalid transaction ID: ${transaction_id}", ("transaction_id", transaction_id_str))
   ...
   getTransactions->set_callback([&] {
      ...
      if (skip_seq_str.empty()) {
         ...
         } OAC_RETHROW_EXCEPTIONS(chain_type_exception, "Invalid Skip Seq: ${skip_seq}", ("skip_seq", skip_seq_str))
         if (num_seq_str.empty()) {
            ...
            } OAC_RETHROW_EXCEPTIONS(chain_type_exception, "Invalid Num Seq: ${num_seq}", ("num_seq", num_seq_str))
...
//                                ->check(CLI::ExistingFile);
   ...
   auto set_abi_callback = [&]() {
      ...
      } OAC_RETHROW_EXCEPTIONS(abi_type_exception,  "Fail to parse ABI JSON")
      if ( shouldSend ) {
   ...
   // Transfer subcommand
   string con = "oac.token";
   string sender;
   string recipient;
   string amount;
   string memo;
   auto transfer = app.add_subcommand("transfer", localized("Transfer OAC from account to account"), false);
   transfer->add_option("sender", sender, localized("The account sending OAC"))->required();
   transfer->add_option("recipient", recipient, localized("The account receiving OAC"))->required();
   transfer->add_option("amount", amount, localized("The amount of OAC to send"))->required();
   ...
   importWallet->set_callback([&wallet_name, &wallet_key_str] {
      ...
      try {
         wallet_key = private_key_type( wallet_key_str );
          ...
          OAC_THROW(private_key_type_exception, "Invalid private key: ${private_key}", ("private_key", wallet_key_str))
      }
   ...
   auto stopKoacd = wallet->add_subcommand("stop", localized("Stop koacd (doesn't work with nodoac)."), false);
   stopKoacd->set_callback([] {
      // wait for koacd to come up
      try_port(uint16_t(std::stoi(parse_url(wallet_url).port)), 2000);

      const auto& v = call(wallet_url, koacd_stop);
      if ( !v.is_object() || v.get_object().size() != 0 ) { //on success koacd responds with empty object
   ...
   sign->set_callback([&] {
      ...
      if( str_chain_id.size() == 0 ) {
         ilog( "grabbing chain_id from nodoac" );
         auto info = get_info();
   ...
   actionsSubcommand->set_callback([&] {
      ...
      if( !data.empty() ) {
         ...
         } OAC_RETHROW_EXCEPTIONS(action_type_exception, "Fail to parse action JSON data='${data}'", ("data", data))
      }
   ...
   trxSubcommand->set_callback([&] {
      ...
      } OAC_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trx_to_push))
   ...
   trxsSubcommand->set_callback([&] {
      ...
      } OAC_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trxsJson))
   ...
   propose_action->set_callback([&] {
      ...
      } OAC_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse permissions JSON '${data}'", ("data",requested_perm))
      fc::variant transaction_perm_var;
      try {
         transaction_perm_var = json_from_file_or_string(transaction_perm);
      } OAC_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse permissions JSON '${data}'", ("data",transaction_perm))
      fc::variant trx_var;
      try {
         trx_var = json_from_file_or_string(proposed_transaction);
      } OAC_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",proposed_transaction))
      ...
      } OAC_RETHROW_EXCEPTIONS(transaction_type_exception, "Wrong requested permissions format: '${data}'", ("data",requested_perm_var));

      vector<permission_level> trxperm;
      try {
         trxperm = transaction_perm_var.as<vector<permission_level>>();
      } OAC_RETHROW_EXCEPTIONS(transaction_type_exception, "Wrong transaction permissions format: '${data}'", ("data",transaction_perm_var));
      ...
      if (accountPermissions.empty()) {
         if (!proposer.empty()) {
            ...
            OAC_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <proposer> or -p)");
         }
      }
      if (proposer.empty()) {
      ...
      send_actions({chain::action{accountPermissions, "oac.msig", "propose", result.get_object()["binargs"].as<bytes>()}});
   });
   ...
   review->set_callback([&] {
      auto result = call(get_table_func, fc::mutable_variant_object("json", true)
                         ...
                         ("table", "proposal")
                         ("table_key", "")
                         ("lower_bound", oac::chain::string_to_name(proposal_name.c_str()))
                         ("upper_bound", "")
                         ("limit", 1)
                         );
      //std::cout << fc::json::to_pretty_string(result) << std::endl;
   ...
   auto approve_or_unapprove = [&](const string& action) {
      ...
      } OAC_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse permissions JSON '${data}'", ("data",perm))
      auto arg = fc::mutable_variant_object()
         ("code", "oac.msig")
         ("action", action)
         ("args", fc::mutable_variant_object()
      ...
      send_actions({chain::action{accountPermissions, "oac.msig", action, result.get_object()["binargs"].as<bytes>()}});
   };
   ...
   cancel->set_callback([&]() {
      ...
      if (accountPermissions.empty()) {
         if (!canceler.empty()) {
            ...
            OAC_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <canceler> or -p)");
         }
      }
      if (canceler.empty()) {
      ...
      auto result = call(json_to_bin_func, arg);
      send_actions({chain::action{accountPermissions, "oac.msig", "cancel", result.get_object()["binargs"].as<bytes>()}});
      }
   );
   ...
   exec->set_callback([&] {
      ...
      if (accountPermissions.empty()) {
         if (!executer.empty()) {
            ...
            OAC_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <executer> or -p)");
         }
      }
      if (executer.empty()) {
      ...
      send_actions({chain::action{accountPermissions, "oac.msig", "exec", result.get_object()["binargs"].as<bytes>()}});
      }
   );

   // system subcommand
   auto system = app.add_subcommand("system", localized("Send oac.system contract action to the blockchain."), false);
