...
string abi_generator::translate_type(const string& type_name) {
  ...
  if (type_name == "unsigned __int128" || type_name == "uint128_t") built_in_type = "uint128";
     ...
     static auto types = oac::chain::common_type_defs();
     auto itr = std::find_if( types.begin(), types.end(),
                              [&type_name]( const oac::chain::type_def& t ) { return t.new_type_name == type_name; } );
     if( itr != types.end()) {
...
void abi_generator::handle_decl(const Decl* decl) { try {
  ...
  // If OACIO_ABI macro was found, check if the current declaration
  ...
  // The current Decl was the type referenced in OACIO_ABI macro
  if( type_has_actions ) return;

  // The current Decl was not the type referenced in OACIO_ABI macro
  ...
  // If OACIO_ABI macro was found, we will only check if the current Decl
