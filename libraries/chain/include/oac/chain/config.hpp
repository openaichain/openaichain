...
namespace oac { namespace chain { namespace config {
...
const static uint64_t system_account_name    = N(oac);
const static uint64_t null_account_name      = N(oac.null);
const static uint64_t producers_account_name = N(oac.prods);
...
const static uint64_t oac_auth_scope       = N(oac.auth);
const static uint64_t oac_all_scope        = N(oac.all);

const static uint64_t active_name = N(active);
const static uint64_t owner_name  = N(owner);
const static uint64_t oac_any_name = N(oac.any);
const static uint64_t oac_code_name = N(oac.code);
...
const static oac::chain::wasm_interface::vm_type default_wasm_runtime = oac::chain::wasm_interface::vm_type::binaryen;
...
} } } // namespace oac::chain::config

constexpr uint64_t OAC_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / oac::chain::config::percent_100;
}

template<typename Number>
Number OAC_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + oac::chain::config::percent_100 - oac::chain::config::percent_1)  / oac::chain::config::percent_100;
}
