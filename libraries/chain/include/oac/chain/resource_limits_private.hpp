...
namespace oac { namespace chain { namespace resource_limits {
   ...
   namespace impl {
      ...
      T operator* (T value, const ratio<T>& r) {
         OAC_ASSERT(r.numerator == T(0) || std::numeric_limits<T>::max() / r.numerator >= value, rate_limiting_state_inconsistent, "Usage exceeds maximum value representable after extending for precision");
         ...
         std::enable_if_t<is_valid_downgrade_cast<LesserIntType,GreaterIntType> && std::is_signed<LesserIntType>::value == std::is_signed<GreaterIntType>::value, LesserIntType>
      {
         ...
         OAC_ASSERT( val >= min && val <= max, rate_limiting_state_inconsistent, "Casting a higher bit integer value ${v} to a lower bit integer value which cannot contain the value, valid range is [${min}, ${max}]", ("v", val)("min", min)("max",max) );
      ...
      struct exponential_moving_average_accumulator
      {
         ...
         void add( uint64_t units, uint32_t ordinal, uint32_t window_size /* must be positive */ )
         {
            ...
            OAC_ASSERT(units <= max_raw_value, rate_limiting_state_inconsistent, "Usage exceeds maximum value representable after extending for precision");
            OAC_ASSERT(std::numeric_limits<decltype(consumed)>::max() - consumed >= units, rate_limiting_state_inconsistent, "Overflow in tracked usage when adding usage!");

            auto value_ex_contrib = downgrade_cast<uint64_t>(integer_divide_ceil((uint128_t)units * Precision, (uint128_t)window_size));
            OAC_ASSERT(std::numeric_limits<decltype(value_ex)>::max() - value_ex >= value_ex_contrib, rate_limiting_state_inconsistent, "Overflow in accumulated value when adding usage!");
   ...
   class resource_limits_config_object : public chainbase::object<resource_limits_config_object_type, resource_limits_config_object> {
      ...
      elastic_limit_parameters cpu_limit_parameters = {OAC_PERCENT(config::default_max_block_cpu_usage, config::default_target_block_cpu_usage_pct), config::default_max_block_cpu_usage, config::block_cpu_usage_average_window_ms / config::block_interval_ms, 1000, {99, 100}, {1000, 999}};
      elastic_limit_parameters net_limit_parameters = {OAC_PERCENT(config::default_max_block_net_usage, config::default_target_block_net_usage_pct), config::default_max_block_net_usage, config::block_size_average_window_ms / config::block_interval_ms, 1000, {99, 100}, {1000, 999}};
...
} } } /// oac::chain::resource_limits

CHAINBASE_SET_INDEX_TYPE(oac::chain::resource_limits::resource_limits_object,        oac::chain::resource_limits::resource_limits_index)
CHAINBASE_SET_INDEX_TYPE(oac::chain::resource_limits::resource_usage_object,         oac::chain::resource_limits::resource_usage_index)
CHAINBASE_SET_INDEX_TYPE(oac::chain::resource_limits::resource_limits_config_object, oac::chain::resource_limits::resource_limits_config_index)
CHAINBASE_SET_INDEX_TYPE(oac::chain::resource_limits::resource_limits_state_object,  oac::chain::resource_limits::resource_limits_state_index)
