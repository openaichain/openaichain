...
#define N(X) oac::chain::string_to_name(#X)
...
namespace std {
   template<> struct hash<oac::chain::name> : private hash<uint64_t> {
      typedef oac::chain::name argument_type;
...
namespace fc {
  class variant;
  void to_variant(const oac::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, oac::chain::name& check);
} // fc


FC_REFLECT( oac::chain::name, (value) )
