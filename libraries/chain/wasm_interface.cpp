...
#include <oac/chain/producer_schedule.hpp>
#include <oac/chain/exceptions.hpp>
#include <boost/core/ignore_unused.hpp>
#include <oac/chain/authorization_manager.hpp>
#include <oac/chain/resource_limits.hpp>
#include <oac/chain/wasm_interface_private.hpp>
#include <oac/chain/wasm_oac_validation.hpp>
#include <oac/chain/wasm_oac_injection.hpp>
...
namespace oac { namespace chain {
   ...
   void wasm_interface::validate(const bytes& code) {
      ...
      try {
         ...
         OAC_ASSERT(false, wasm_serialization_error, e.message.c_str());
...
class privileged_api : public context_aware_api {
   public:
      ...
      void set_resource_limits( account_name account, int64_t ram_bytes, int64_t net_weight, int64_t cpu_weight) {
         OAC_ASSERT(ram_bytes >= -1, wasm_execution_error, "invalid value for ram resource limit expected [-1,INT64_MAX]");
         OAC_ASSERT(net_weight >= -1, wasm_execution_error, "invalid value for net resource weight expected [-1,INT64_MAX]");
         OAC_ASSERT(cpu_weight >= -1, wasm_execution_error, "invalid value for cpu resource weight expected [-1,INT64_MAX]");
      ...
      int64_t set_proposed_producers( array_ptr<char> packed_producer_schedule, size_t datalen) {
         ...
         OAC_ASSERT(producers.size() <= config::max_producers, wasm_execution_error, "Producer schedule exceeds the maximum producer count for this chain");
         // check that producers are unique
         std::set<account_name> unique_producers;
         for (const auto& p: producers) {
            OAC_ASSERT( context.is_account(p.producer_name), wasm_execution_error, "producer schedule includes a nonexisting account" );
            OAC_ASSERT( p.block_signing_key.valid(), wasm_execution_error, "producer schedule includes an invalid key" );
            unique_producers.insert(p.producer_name);
         }
         OAC_ASSERT( producers.size() == unique_producers.size(), wasm_execution_error, "duplicate producer name in producer schedule" );
...
class softfloat_api : public context_aware_api {
   public:
      ...
      float _oac_f32_max( float af, float bf ) {
         float32_t a = to_softfloat32(af);
         float32_t b = to_softfloat32(bf);
         if (is_nan(a)) {
            return af;
      ...
      float _oac_f32_copysign( float af, float bf ) {
         float32_t a = to_softfloat32(af);
         float32_t b = to_softfloat32(bf);
      ...
      // float unops
      float _oac_f32_abs( float af ) {
         float32_t a = to_softfloat32(af);
         a.v &= ~(1 << 31);
         return from_softfloat32(a);
      }
      float _oac_f32_neg( float af ) {
         float32_t a = to_softfloat32(af);
      ...
      float _oac_f32_nearest( float af ) {
         float32_t a = to_softfloat32(af);
         int e = a.v>>23 & 0xff;
         int s = a.v>>31;
         float32_t y;
      ...
      // float relops
      bool _oac_f32_eq( float a, float b ) {  return f32_eq( to_softfloat32(a), to_softfloat32(b) ); }
      bool _oac_f32_ne( float a, float b ) { return !f32_eq( to_softfloat32(a), to_softfloat32(b) ); }
      bool _oac_f32_lt( float a, float b ) { return f32_lt( to_softfloat32(a), to_softfloat32(b) ); }
      bool _oac_f32_le( float a, float b ) { return f32_le( to_softfloat32(a), to_softfloat32(b) ); }
      bool _oac_f32_gt( float af, float bf ) {
         float32_t a = to_softfloat32(af);
         float32_t b = to_softfloat32(bf);
         if (is_nan(a))
            return false;
      ...
      bool _oac_f32_ge( float af, float bf ) {
         float32_t a = to_softfloat32(af);
         float32_t b = to_softfloat32(bf);
         if (is_nan(a))
            return false;
      ...
      double _oac_f64_max( double af, double bf ) {
         float64_t a = to_softfloat64(af);
         float64_t b = to_softfloat64(bf);
         if (is_nan(a))
            return af;
      ...
      double _oac_f64_copysign( double af, double bf ) {
         float64_t a = to_softfloat64(af);
         float64_t b = to_softfloat64(bf);
      ...
      // double unops
      double _oac_f64_abs( double af ) {
         float64_t a = to_softfloat64(af);
         a.v &= ~(uint64_t(1) << 63);
         return from_softfloat64(a);
      }
      double _oac_f64_neg( double af ) {
         float64_t a = to_softfloat64(af);
      ...
      double _oac_f64_sqrt( double a ) {
         float64_t ret = f64_sqrt( to_softfloat64(a) );
         return from_softfloat64(ret);
      }
      // ceil, floor, trunc and nearest are lifted from libc
      double _oac_f64_ceil( double af ) {
         float64_t a = to_softfloat64( af );
         float64_t ret;
         int e = a.v >> 52 & 0x7ff;
         float64_t y;
      ...
      double _oac_f64_floor( double af ) {
         float64_t a = to_softfloat64( af );
         float64_t ret;
         int e = a.v >> 52 & 0x7FF;
         float64_t y;
      ...
      double _oac_f64_trunc( double af ) {
         float64_t a = to_softfloat64( af );
      ...
      double _oac_f64_nearest( double af ) {
         float64_t a = to_softfloat64( af );
         int e = (a.v >> 52 & 0x7FF);
         int s = a.v >> 63;
         float64_t y;
      ...
      // double relops
      bool _oac_f64_eq( double a, double b ) { return f64_eq( to_softfloat64(a), to_softfloat64(b) ); }
      bool _oac_f64_ne( double a, double b ) { return !f64_eq( to_softfloat64(a), to_softfloat64(b) ); }
      bool _oac_f64_lt( double a, double b ) { return f64_lt( to_softfloat64(a), to_softfloat64(b) ); }
      bool _oac_f64_le( double a, double b ) { return f64_le( to_softfloat64(a), to_softfloat64(b) ); }
      bool _oac_f64_gt( double af, double bf ) {
         float64_t a = to_softfloat64(af);
         float64_t b = to_softfloat64(bf);
         if (is_nan(a))
            return false;
      ...
      bool _oac_f64_ge( double af, double bf ) {
         float64_t a = to_softfloat64(af);
         float64_t b = to_softfloat64(bf);
         if (is_nan(a))
            return false;
      ...
      double _oac_f32_promote( float a ) {
         return from_softfloat64(f32_to_f64( to_softfloat32(a)) );
      }
      float _oac_f64_demote( double a ) {
         return from_softfloat32(f64_to_f32( to_softfloat64(a)) );
      }
      int32_t _oac_f32_trunc_i32s( float af ) {
         float32_t a = to_softfloat32(af);
         if (_oac_f32_ge(af, 2147483648.0f) || _oac_f32_lt(af, -2147483648.0f))
            FC_THROW_EXCEPTION( oac::chain::wasm_execution_error, "Error, f32.convert_s/i32 overflow" );

         if (is_nan(a))
            FC_THROW_EXCEPTION( oac::chain::wasm_execution_error, "Error, f32.convert_s/i32 unrepresentable");
         return f32_to_i32( to_softfloat32(_oac_f32_trunc( af )), 0, false );
      }
      int32_t _oac_f64_trunc_i32s( double af ) {
         float64_t a = to_softfloat64(af);
         if (_oac_f64_ge(af, 2147483648.0) || _oac_f64_lt(af, -2147483648.0))
            FC_THROW_EXCEPTION( oac::chain::wasm_execution_error, "Error, f64.convert_s/i32 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( oac::chain::wasm_execution_error, "Error, f64.convert_s/i32 unrepresentable");
         return f64_to_i32( to_softfloat64(_oac_f64_trunc( af )), 0, false );
      }
      uint32_t _oac_f32_trunc_i32u( float af ) {
         float32_t a = to_softfloat32(af);
         if (_oac_f32_ge(af, 4294967296.0f) || _oac_f32_le(af, -1.0f))
            FC_THROW_EXCEPTION( oac::chain::wasm_execution_error, "Error, f32.convert_u/i32 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( oac::chain::wasm_execution_error, "Error, f32.convert_u/i32 unrepresentable");
         return f32_to_ui32( to_softfloat32(_oac_f32_trunc( af )), 0, false );
      }
      uint32_t _oac_f64_trunc_i32u( double af ) {
         float64_t a = to_softfloat64(af);
         if (_oac_f64_ge(af, 4294967296.0) || _oac_f64_le(af, -1.0))
            FC_THROW_EXCEPTION( oac::chain::wasm_execution_error, "Error, f64.convert_u/i32 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( oac::chain::wasm_execution_error, "Error, f64.convert_u/i32 unrepresentable");
         return f64_to_ui32( to_softfloat64(_oac_f64_trunc( af )), 0, false );
      }
      int64_t _oac_f32_trunc_i64s( float af ) {
         float32_t a = to_softfloat32(af);
         if (_oac_f32_ge(af, 9223372036854775808.0f) || _oac_f32_lt(af, -9223372036854775808.0f))
            FC_THROW_EXCEPTION( oac::chain::wasm_execution_error, "Error, f32.convert_s/i64 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( oac::chain::wasm_execution_error, "Error, f32.convert_s/i64 unrepresentable");
         return f32_to_i64( to_softfloat32(_oac_f32_trunc( af )), 0, false );
      }
      int64_t _oac_f64_trunc_i64s( double af ) {
         float64_t a = to_softfloat64(af);
         if (_oac_f64_ge(af, 9223372036854775808.0) || _oac_f64_lt(af, -9223372036854775808.0))
            FC_THROW_EXCEPTION( oac::chain::wasm_execution_error, "Error, f64.convert_s/i64 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( oac::chain::wasm_execution_error, "Error, f64.convert_s/i64 unrepresentable");

         return f64_to_i64( to_softfloat64(_oac_f64_trunc( af )), 0, false );
      }
      uint64_t _oac_f32_trunc_i64u( float af ) {
         float32_t a = to_softfloat32(af);
         if (_oac_f32_ge(af, 18446744073709551616.0f) || _oac_f32_le(af, -1.0f))
            FC_THROW_EXCEPTION( oac::chain::wasm_execution_error, "Error, f32.convert_u/i64 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( oac::chain::wasm_execution_error, "Error, f32.convert_u/i64 unrepresentable");
         return f32_to_ui64( to_softfloat32(_oac_f32_trunc( af )), 0, false );
      }
      uint64_t _oac_f64_trunc_i64u( double af ) {
         float64_t a = to_softfloat64(af);
         if (_oac_f64_ge(af, 18446744073709551616.0) || _oac_f64_le(af, -1.0))
            FC_THROW_EXCEPTION( oac::chain::wasm_execution_error, "Error, f64.convert_u/i64 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( oac::chain::wasm_execution_error, "Error, f64.convert_u/i64 unrepresentable");
         return f64_to_ui64( to_softfloat64(_oac_f64_trunc( af )), 0, false );
      }
      float _oac_i32_to_f32( int32_t a )  {
         return from_softfloat32(i32_to_f32( a ));
      }
      float _oac_i64_to_f32( int64_t a ) {
         return from_softfloat32(i64_to_f32( a ));
      }
      float _oac_ui32_to_f32( uint32_t a ) {
         return from_softfloat32(ui32_to_f32( a ));
      }
      float _oac_ui64_to_f32( uint64_t a ) {
         return from_softfloat32(ui64_to_f32( a ));
      }
      double _oac_i32_to_f64( int32_t a ) {
         return from_softfloat64(i32_to_f64( a ));
      }
      double _oac_i64_to_f64( int64_t a ) {
         return from_softfloat64(i64_to_f64( a ));
      }
      double _oac_ui32_to_f64( uint32_t a ) {
         return from_softfloat64(ui32_to_f64( a ));
      }
      double _oac_ui64_to_f64( uint64_t a ) {
...
class crypto_api : public context_aware_api {
   public:
      ...
      template<class Encoder> auto encode(char* data, size_t datalen) {
         Encoder e;
         const size_t bs = oac::chain::config::hashing_checktime_block_size;
         while ( datalen > bs ) {
            e.write( data, bs );
            data += bs;
            datalen -= bs;
...
class permission_api : public context_aware_api {
   public:
      ...
      {
         OAC_ASSERT( delay_us <= static_cast<uint64_t>(std::numeric_limits<int64_t>::max()),
...
public:
   ...
   // Kept as intrinsic rather than implementing on WASM side (using oac_assert_message and strlen) because strlen is faster on native side.
   void oac_assert( bool condition, null_terminated_ptr msg ) {
      if( BOOST_UNLIKELY( !condition ) ) {
         std::string message( msg );
         edump((message));
         OAC_THROW( oac_assert_message_exception, "assertion failure with message: ${s}", ("s",message) );
      }
   }

   void oac_assert_message( bool condition, array_ptr<const char> msg, size_t msg_len ) {
      if( BOOST_UNLIKELY( !condition ) ) {
   ...
   void oac_assert_code( bool condition, uint64_t error_code ) {
      if( BOOST_UNLIKELY( !condition ) ) {
         edump((error_code));
         OAC_THROW( oac_assert_code_exception,
                    "assertion failure with error code: ${error_code}", ("error_code", error_code) );
      }
   }

   void oac_exit(int32_t code) {
...
#define DB_API_METHOD_WRAPPERS_FLOAT_SECONDARY(IDX, TYPE)\
      int db_##IDX##_store( uint64_t scope, uint64_t table, uint64_t payer, uint64_t id, const TYPE& secondary ) {\
         OAC_ASSERT( !softfloat_api::is_nan( secondary ), transaction_exception, "NaN is not an allowed value for a secondary key" );\
         return context.IDX.store( scope, table, payer, id, secondary );\
      }\
      void db_##IDX##_update( int iterator, uint64_t payer, const TYPE& secondary ) {\
         OAC_ASSERT( !softfloat_api::is_nan( secondary ), transaction_exception, "NaN is not an allowed value for a secondary key" );\
...
class memory_api : public context_aware_api {
   public:
      ...
      char* memcpy( array_ptr<char> dest, array_ptr<const char> src, size_t length) {
         OAC_ASSERT((std::abs((ptrdiff_t)dest.value - (ptrdiff_t)src.value)) >= length,
...
/*
 * This api will be removed with fix for `oac #2561`
 */
...
REGISTER_INTRINSICS(context_free_system_api,
   (abort,                void()              )
   (oac_assert,         void(int, int)      )
   (oac_assert_message, void(int, int, int) )
   (oac_assert_code,    void(int, int64_t)  )
   (oac_exit,           void(int)           )
);
...
REGISTER_INJECTED_INTRINSICS(softfloat_api,
      (_oac_f32_add,       float(float, float)    )
      (_oac_f32_sub,       float(float, float)    )
      (_oac_f32_mul,       float(float, float)    )
      (_oac_f32_div,       float(float, float)    )
      (_oac_f32_min,       float(float, float)    )
      (_oac_f32_max,       float(float, float)    )
      (_oac_f32_copysign,  float(float, float)    )
      (_oac_f32_abs,       float(float)           )
      (_oac_f32_neg,       float(float)           )
      (_oac_f32_sqrt,      float(float)           )
      (_oac_f32_ceil,      float(float)           )
      (_oac_f32_floor,     float(float)           )
      (_oac_f32_trunc,     float(float)           )
      (_oac_f32_nearest,   float(float)           )
      (_oac_f32_eq,        int(float, float)      )
      (_oac_f32_ne,        int(float, float)      )
      (_oac_f32_lt,        int(float, float)      )
      (_oac_f32_le,        int(float, float)      )
      (_oac_f32_gt,        int(float, float)      )
      (_oac_f32_ge,        int(float, float)      )
      (_oac_f64_add,       double(double, double) )
      (_oac_f64_sub,       double(double, double) )
      (_oac_f64_mul,       double(double, double) )
      (_oac_f64_div,       double(double, double) )
      (_oac_f64_min,       double(double, double) )
      (_oac_f64_max,       double(double, double) )
      (_oac_f64_copysign,  double(double, double) )
      (_oac_f64_abs,       double(double)         )
      (_oac_f64_neg,       double(double)         )
      (_oac_f64_sqrt,      double(double)         )
      (_oac_f64_ceil,      double(double)         )
      (_oac_f64_floor,     double(double)         )
      (_oac_f64_trunc,     double(double)         )
      (_oac_f64_nearest,   double(double)         )
      (_oac_f64_eq,        int(double, double)    )
      (_oac_f64_ne,        int(double, double)    )
      (_oac_f64_lt,        int(double, double)    )
      (_oac_f64_le,        int(double, double)    )
      (_oac_f64_gt,        int(double, double)    )
      (_oac_f64_ge,        int(double, double)    )
      (_oac_f32_promote,    double(float)         )
      (_oac_f64_demote,     float(double)         )
      (_oac_f32_trunc_i32s, int(float)            )
      (_oac_f64_trunc_i32s, int(double)           )
      (_oac_f32_trunc_i32u, int(float)            )
      (_oac_f64_trunc_i32u, int(double)           )
      (_oac_f32_trunc_i64s, int64_t(float)        )
      (_oac_f64_trunc_i64s, int64_t(double)       )
      (_oac_f32_trunc_i64u, int64_t(float)        )
      (_oac_f64_trunc_i64u, int64_t(double)       )
      (_oac_i32_to_f32,     float(int32_t)        )
      (_oac_i64_to_f32,     float(int64_t)        )
      (_oac_ui32_to_f32,    float(int32_t)        )
      (_oac_ui64_to_f32,    float(int64_t)        )
      (_oac_i32_to_f64,     double(int32_t)       )
      (_oac_i64_to_f64,     double(int64_t)       )
      (_oac_ui32_to_f64,    double(int32_t)       )
      (_oac_ui64_to_f64,    double(int64_t)       )
);
...
std::istream& operator>>(std::istream& in, wasm_interface::vm_type& runtime) {
   ...
   in >> s;
   if (s == "wavm")
      runtime = oac::chain::wasm_interface::vm_type::wavm;
   else if (s == "binaryen")
      runtime = oac::chain::wasm_interface::vm_type::binaryen;
   else
