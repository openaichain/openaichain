...
#include <oac/chain/block_summary_object.hpp>
...
#define SET_APP_HANDLER( receiver, contract, action) \
   ...
   SET_APP_HANDLER( oac, oac, newaccount );
   SET_APP_HANDLER( oac, oac, setcode );
   SET_APP_HANDLER( oac, oac, setabi );
   SET_APP_HANDLER( oac, oac, updateauth );
   SET_APP_HANDLER( oac, oac, deleteauth );
   SET_APP_HANDLER( oac, oac, linkauth );
   SET_APP_HANDLER( oac, oac, unlinkauth );
/*
   SET_APP_HANDLER( oac, oac, postrecovery );
   SET_APP_HANDLER( oac, oac, passrecovery );
   SET_APP_HANDLER( oac, oac, vetorecovery );
*/

   SET_APP_HANDLER( oac, oac, canceldelay );
...
//         edump((s->block_num)(s->block->previous)(log_head->id()));
   ...
   void initialize_fork_db() {
      ...
      producer_schedule_type initial_schedule{ 0, {{N(oac), conf.genesis.initial_key}} };
   ...
   void create_native_account( account_name name, const authority& owner, const authority& active, bool is_privileged = false ) {
      db.create<account_object>([&](auto& a) {
         ...
         if( name == config::system_account_name ) {
            a.set_abi(oac_contract_abi(abi_def()));
         }
      });
  ...
  //    ilog((fc::json::to_pretty_string(*pending->_pending_block_state->block)));
      ...
      try {
         ...
         for( const auto& receipt : b->transactions ) {
            ...
            if( receipt.trx.contains<packed_transaction>() ) {
               ...
               OAC_ASSERT( false, block_validate_exception, "encountered unexpected receipt type" );
            }
            ...
            OAC_ASSERT( pending->_pending_block_state->block->transactions.size() > 0,
                        block_validate_exception, "expected a receipt",
                        ("block", *b)("expected_receipt", receipt)
                      );
            OAC_ASSERT( pending->_pending_block_state->block->transactions.size() == num_pending_receipts + 1,
            ...
            OAC_ASSERT( r == static_cast<const transaction_receipt_header&>(receipt),
   ...
   void finalize_block()
   {
      ...
      */
      ...
      uint64_t CPU_TARGET = OAC_PERCENT(chain_config.max_block_cpu_usage, chain_config.target_block_cpu_usage_pct);
      resource_limits.set_block_parameters(
         ...
         {OAC_PERCENT(chain_config.max_block_net_usage, chain_config.target_block_net_usage_pct), chain_config.max_block_net_usage, config::block_size_average_window_ms / config::block_interval_ms, max_virtual_mult, {99, 100}, {1000, 999}}
      );
   ...
   void check_actor_list( const flat_set<account_name>& actors )const {
      if( conf.actor_whitelist.size() > 0 ) {
         ...
         OAC_ASSERT( excluded.size() == 0, actor_whitelist_exception,
         ...
         OAC_ASSERT( blacklisted.size() == 0, actor_blacklist_exception,
   ...
   void check_contract_list( account_name code )const {
      if( conf.contract_whitelist.size() > 0 ) {
         OAC_ASSERT( conf.contract_whitelist.find( code ) != conf.contract_whitelist.end(),
                     contract_whitelist_exception,
                     "account '${code}' is not on the contract whitelist", ("code", code)
                   );
      } else if( conf.contract_blacklist.size() > 0 ) {
         OAC_ASSERT( conf.contract_blacklist.find( code ) == conf.contract_blacklist.end(),
   ...
   void check_action_list( account_name code, action_name action )const {
      if( conf.action_blacklist.size() > 0 ) {
         OAC_ASSERT( conf.action_blacklist.find( std::make_pair(code, action) ) == conf.action_blacklist.end(),
   ...
   void validate_tapos( const transaction& trx )const {
      ...
      OAC_ASSERT(trx.verify_reference_block(tapos_block_summary.block_id), invalid_ref_block_exception,
...
block_id_type controller::get_block_id_for_num( uint32_t block_num )const { try {
   ...
   OAC_ASSERT( BOOST_LIKELY( signed_blk != nullptr ), unknown_block_exception,
...
void controller::validate_referenced_accounts( const transaction& trx )const {
   for( const auto& a : trx.context_free_actions ) {
      ...
      OAC_ASSERT( code != nullptr, transaction_exception,
                  "action's code account '${account}' does not exist", ("account", a.account) );
      OAC_ASSERT( a.authorization.size() == 0, transaction_exception,
   ...
   for( const auto& a : trx.actions ) {
      ...
      for( const auto& auth : a.authorization ) {
         ...
         OAC_ASSERT( actor  != nullptr, transaction_exception,
                     "action's authorizing actor '${account}' does not exist", ("account", auth.actor) );
         OAC_ASSERT( my->authorization.find_permission(auth) != nullptr, transaction_exception,
                     "action's authorizations include a non-existent permission: {permission}",
                     ("permission", auth) );
      }
   }
   OAC_ASSERT( one_auth, tx_no_auths, "transaction must have at least one authorization" );
}

void controller::validate_expiration( const transaction& trx )const { try {
   const auto& chain_configuration = get_global_properties().configuration;

   OAC_ASSERT( time_point(trx.expiration) >= pending_block_time(),
               expired_tx_exception,
               "transaction has expired, "
               "expiration is ${trx.expiration} and pending block time is ${pending_block_time}",
               ("trx.expiration",trx.expiration)("pending_block_time",pending_block_time()));
   OAC_ASSERT( time_point(trx.expiration) <= pending_block_time() + fc::seconds(chain_configuration.max_transaction_lifetime),
