...
#include <oac/chain/authority_checker.hpp>
...
namespace oac { namespace chain {
   ...
   } OAC_RETHROW_EXCEPTIONS( chain::permission_query_exception, "Failed to retrieve permission: ${level}", ("level", level) ) }
                                                                             ...
                                                                             )const
   {
      ...
      try {
         ...
         if( *linked_permission == config::oac_any_name )
            return optional<permission_name>();
   ...
   {
      OAC_ASSERT( auths.size() == 1, irrelevant_auth_exception,
                  "updateauth action should only have one declared authorization" );
      const auto& auth = auths[0];
      OAC_ASSERT( auth.actor == update.account, irrelevant_auth_exception,
      ...
      OAC_ASSERT( get_permission(auth).satisfies( *min_permission,
                                                             ...
                                                             )const
   {
      ...
      const auto& auth = auths[0];
      OAC_ASSERT( auth.actor == del.account, irrelevant_auth_exception,
                                                           ...
                                                           )const
   {
      ...
      const auto& auth = auths[0];
      OAC_ASSERT( auth.actor == link.account, irrelevant_auth_exception,
                  "the owner of the linked permission needs to be the actor of the declared authorization" );

      OAC_ASSERT( link.type != updateauth::get_name(),  action_validate_exception,
                  "Cannot link oac::updateauth to a minimum permission" );
      OAC_ASSERT( link.type != deleteauth::get_name(),  action_validate_exception,
                  "Cannot link oac::deleteauth to a minimum permission" );
      OAC_ASSERT( link.type != linkauth::get_name(),    action_validate_exception,
                  "Cannot link oac::linkauth to a minimum permission" );
      OAC_ASSERT( link.type != unlinkauth::get_name(),  action_validate_exception,
                  "Cannot link oac::unlinkauth to a minimum permission" );
      OAC_ASSERT( link.type != canceldelay::get_name(), action_validate_exception,
                  "Cannot link oac::canceldelay to a minimum permission" );

      const auto linked_permission_name = lookup_minimum_permission(link.account, link.code, link.type);

      if( !linked_permission_name ) // if action is linked to oac.any permission
         return;

      OAC_ASSERT( get_permission(auth).satisfies( get_permission({link.account, *linked_permission_name}),
                                                  _db.get_index<permission_index>().indices()              ),
                  irrelevant_auth_exception,
                                                             ...
                                                             )const
   {
      ...
      const auto& auth = auths[0];
      OAC_ASSERT( auth.actor == unlink.account, irrelevant_auth_exception,
                  "the owner of the linked permission needs to be the actor of the declared authorization" );

      const auto unlinked_permission_name = lookup_linked_permission(unlink.account, unlink.code, unlink.type);
      OAC_ASSERT( unlinked_permission_name.valid(), transaction_exception,
      ...
      if( *unlinked_permission_name == config::oac_any_name )
         return;

      OAC_ASSERT( get_permission(auth).satisfies( get_permission({unlink.account, *unlinked_permission_name}),
                                                  _db.get_index<permission_index>().indices()                  ),
                  irrelevant_auth_exception,
                                                                          ...
                                                                          )const
   {
      ...
      OAC_ASSERT( get_permission(auth).satisfies( get_permission(cancel.canceling_auth),
                                                  _db.get_index<permission_index>().indices() ),
                  irrelevant_auth_exception,
      ...
      OAC_ASSERT( found, action_validate_exception,
                                             ...
                                             )const
   {
      ...
      for( const auto& act : actions ) {
         ...
         for( const auto& declared_auth : act.authorization ) {
            ...
            if( !special_case ) {
               ...
               if( min_permission_name ) { // since special cases were already handled, it should only be false if the permission is oac.any
                  const auto& min_permission = get_permission({declared_auth.actor, *min_permission_name});
                  OAC_ASSERT( get_permission(declared_auth).satisfies( min_permission,
                                                                       _db.get_index<permission_index>().indices() ),
                              irrelevant_auth_exception,
      ...
      for( const auto& p : permissions_to_satisfy ) {
         ...
         OAC_ASSERT( checker.satisfied( p.first, p.second ), unsatisfied_authorization,
      ...
      if( !allow_unused_keys ) {
         OAC_ASSERT( checker.all_keys_used(), tx_irrelevant_sig,
                                             ...
                                             )const
   {
      ...
      OAC_ASSERT( checker.satisfied({account, permission}), unsatisfied_authorization,
                                                                     ...
                                                                     )const
   {
      ...
      for (const auto& act : trx.actions ) {
         for (const auto& declared_auth : act.authorization) {
            OAC_ASSERT( checker.satisfied(declared_auth), unsatisfied_authorization,
